...
| P5 | §4.4, l.459 | One station-day (URUM, DOY 365) where PPPx did not converge under either weighting moved the Direct STEC elev-weighted **mean** by 25% (2.283 m → 1.706 m, 2026-09-14 reading) while leaving the median unmoved. **2026-09-18: this row is now excluded upstream** by `positioning_coverage`'s solver-failure rule (see `positioning_reporting.md`), so the with/without comparison is historical — current `common_set.csv` reads elev mean 1.688 m / median 0.913 m over N=10,673 | `weighting_ablation` | `common_set.csv`'s `elev_mean` column (2.2828) for the pre-exclusion "with" value; the "without" value (1.706) was the mean after dropping that one row by hand, not itself a separate declared-stage column | same population as P1 | pre-2026-09-18: 10,387, minus 1 for the "without" figure; current population already excludes the row (N=10,673) | n/a — a single genuine PPPx solve failure, now excluded by the 2026-09-18 solver-failure rule rather than an outcome-based outlier rule | mean (used only to illustrate why the table reports medians) | elev |
