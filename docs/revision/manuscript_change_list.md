...
| # | Location | Manuscript says | Artifact said (2026-09-14 snapshot) | Population | Status | Action |
|---|---|---|---|---|---|---|
| 32 | l.451 (\add) | Weighting ablation: Direct STEC 3D RMS 1.156→1.121 m switching elev→iono, "an improvement of 3.0%", restricted to 27,205 station-days solved under both schemes | **Updated 2026-09-15, supersedes this row's own 2026-09-14 reading below.** Direct STEC elev 0.9204→iono 0.8914 m (**median**, `gain_median_%`), **+3.2%**, N=10,387 — the same 4-method × 2-weighting common set Table 5 uses, and now `weighting_ablation`'s own `canonical_for="Table 7"` output, not a self-comparison pairing. (Prior 2026-09-14 reading, itself already superseding the manuscript's 27,205/+3.0%: Direct STEC elev 1.5966→iono 1.5367 m, +3.75% mean, N=10,366 paired.) | `weighting_ablation/rebuilt/common_set.csv` | **VALUE MOVED** — direction and rough magnitude (~3%) hold across every reading; N and methodology (mean, self-paired → median, common-set) both changed since this row was first written | Requote N=10,387 and the median gain +3.2%. This table now also carries a Pretrained Direct STEC row (+4.4%) with no manuscript-table counterpart to compare against. |
| 33 | l.451 (\add) | VTEC + Mapping degrades 1.580→1.624 m under iono weighting | **Updated 2026-09-15**: VTEC elev 1.2425→iono 1.3255 m (median), **−6.7%**, N=10,387 (common set, `weighting_ablation/rebuilt/common_set.csv`) — still degrades, larger in magnitude than the 2026-09-14 reading (−0.05% mean, N=10,640 paired) it supersedes. | `weighting_ablation/rebuilt/common_set.csv` | VALUE MOVED (direction holds, magnitude larger under the current common-set/median reading) | Requote −6.7%, N=10,387 |
| 34 | l.451 (\add) | IGS GIM + Mapping "unchanged at 1.630" | **Updated 2026-09-15**: GIM elev 1.0893→iono 1.0969 m (median), **−0.7%**, N=10,387 (common set) — still "essentially flat", superseding the 2026-09-14 reading (−0.08%, N=10,733 paired). | `weighting_ablation/rebuilt/common_set.csv` | UNCHANGED (still reads as "essentially flat" under any population/statistic tried so far) | none, or requote −0.7%/N=10,387 for precision |
...
|---|---|---|---|---|
| 41 | l.498 | "replacing standard elevation-dependent weighting with predicted-uncertainty weighting improves the 3D RMS error by approximately 3%" | VALUE MOVED (minor) — current median +3.2%, N=10,387 common set, same as #32 (2026-09-15 update) | Requote alongside #32's fix |
...
43 stages stale, and `verification/gate_f_figures.py` reports 10/10 MATCH. *(Both counts are as of 2026-09-14 and have since moved: the pipeline is at 45 stages and the gate at 14/14, four of the new checks being the Figures 5-8 independent re-derivation — see `results_register.md` Item I. The claim this sentence supports, that the two stages had been run against the current population, still holds.)*
