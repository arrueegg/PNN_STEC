...
| R1.8 | Observation-derived oracle/near-oracle PPP benchmark | Nothing — "oracle" does not appear anywhere in the manuscript | **ANSWERED IN LETTER ONLY** | `oracle_benchmark` (`canonical_for="R1.8 observation-derived oracle PPP benchmark"`, set 2026-09-16); re-run 2026-09-14, methodology changed again 2026-09-16 (commit 66fe4bd) to match the positioning tables' population and outlier treatment — `rebuilt` is now 5,442 paired station-days (242/242 days, 52 stations), down from the intermediate 5,514 because of that restriction, not any data loss; see the response letter's rewritten R1.8 | N/A — absent from paper. The evidence side is resolved and quotable (Direct STEC 11.6× the oracle floor, median); only the manuscript side is still unwritten |
