...
### An open question this document did not resolve - resolved 2026-08-24

`stages.py`'s `POSITIONING` constant used to point at `positioning_runs/full_coverage/`
while CLAUDE.md's canonical-results table and `stec.runs.migrate`'s encoding of it named
`positioning_comparison_3way` as "Positioning, Figs 12/13/A1/A2 + Table 5" - both trees
existed on disk holding the same 35,652 rows, and neither was being regenerated by
anything.

Neither was actually right: `full_coverage/` was what the *pre-rebuild*
`src/analysis/positioning_coverage.py` wrote directly, and it was already what every
downstream consumer's own `canonical_positioning_summary()` preferred - the restructure
that created this layout moved `positioning_coverage`'s default output to
`analyses/positioning_coverage/rebuilt/` without repointing those readers, so
`full_coverage/` silently stopped being regenerated. `comparison_3way/` was always the
narrower, frozen published run. The fix (during the recovery-sweep recompute session) was
neither tree: `POSITIONING` and all three duplicated `canonical_positioning_summary()`
copies now point at `positioning_coverage`'s real, current output, and the stage was moved
ahead of its five consumers in `stages.py` so the registry's ordering check enforces the
dependency. Both legacy trees are marked superseded (`.superseded.json` sidecars) rather
than deleted. See CLAUDE.md's canonical-results table for the resulting numbers.
