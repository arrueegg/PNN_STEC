...
The published Table 5 compares four corrections over *different* populations.
After the 10 m outlier rule the counts are gim_iono 10,809 against STEC_iono
8,280, VTEC_iono 8,266 and Pretrained_STEC_iono 8,195: the IGS GIM is solved for
a median of 45 stations per day where every machine-learning method manages 35.

The cause is upstream and legitimate. Those ~2,810 extra station-days are
stations absent from the STEC database on that day, so no ML correction can
exist for them; they are predominantly equatorial (NKLG, CHPG, KOUG, CPVG, FAA1,
LMMF, PTAG, WARK) and they are hard - 2.24 m against 1.40 m on the shared days.
Comparing methods across them therefore mixes a population effect into the
reported improvement, which is what this analysis removes by intersecting.

Sources, all of which agree where they overlap (max |delta error_3d_rms| = 0.0):

* `positioning_comparison_3way/` - the paper's run, 242 dates, the only tree
  carrying Pretrained_STEC_iono
* `positioning_20260216_2052/` - the weighting ablation, 245 dates, six arms
* the pretrained *elevation* arm, read from its per-day summaries because it
  post-dates both trees

Everything is restricted to the 242 dates of the 3way tree, so the ablation's
three extra days cannot leak in.

Usage::

    python src/analysis/common_set_positioning.py
"""
...
ARM_ORDER = [
    "Direct STEC", "Pretrained Direct STEC", "VTEC + Mapping", "IGS GIM + Mapping",
]
...
def load_pretrained_elev(experiment: Path) -> pd.DataFrame:
    ...
    `daily_summary.csv` is the elevation-weighted output (the uncertainty arm
    writes `daily_summary_iono.csv`), and inside it the model rows are labelled
    `model` - run_pipeline renames them only when it aggregates, which this arm
    never went through.
    """
    files = sorted(experiment.glob("positioning/results/2024*/daily_summary.csv"))
    if not files:
        logger.warning(f"⚠️  no elevation-weighted summaries under {experiment}")
...
def build(three_way: Path, ablation: Path, experiment: Path) -> dict:
    ...
    logger.info(f"{len(arms)} arm(s); {len(common):,} station-days solved under all of them")

    keyed = runs.set_index(["station", "doy"])
    common_runs = keyed[keyed.index.isin(common)].reset_index()

    summary = (
        common_runs.groupby("arm")
        .agg(
    ...
    summary["lost_to_intersection"] = per_arm_before.reindex(summary.index) - summary["station_days"]

    # Improvement over the uncertainty-weighted GIM, three ways. Ratio-of-means is
    # what the table quotes; the paired statistics answer "is it better on a
    # typical station-day", which a ratio of aggregates cannot.
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        type=Path,
        default=Path(
            "experiments/Pretrain_STEC_BayesianResNetSTEC_h1024_l4_nh4_v128x4_g32x2_"
            "lr1e-3_bs1024_GNLL_Adam_ReduceLROnPlateau_sub500K_SH5_ps0.1_kl5w0.1_lw1e-1_SWI"
        ),
    )
    parser.add_argument(
        "--output_dir", type=Path, default=Path("multiday_results/common_set_positioning")
    )
    ...
    print(f"=== Table 5 on the common set: {result['common_station_days']:,} station-days "
          f"solved under all {result['arms']} arm(s) ===\n")
    print(
        summary[["station_days", "rms_3d_mean", "rms_3d_median", "rms_2d_mean", "up_mean"]]
        .round(3)
        .to_string()
    )
    print(f"\n--- improvement over {BASELINE_ARM} ---")
    print(
        summary[["gain_ratio_of_means_pct", "gain_paired_mean_pct",
                 "gain_paired_median_pct", "win_rate_pct"]]
        .round(1)
        .to_string()
    )
    print("\n--- station-days each arm loses to the intersection ---")
    print(summary["lost_to_intersection"].astype(int).to_string())
    logger.info(f"💾 {args.output_dir / 'table5_common_set.csv'}")
