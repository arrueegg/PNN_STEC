...
def create_temporal_metrics_summaries(df: pd.DataFrame, output_dir: str = "plots") -> None:
    """
    ...
    def write_metrics_file(filename, title, metrics_dict, description=""):
        ...
        with open(filepath, 'w') as f:
            ...
            f.write(f"Pearson Correlation:   {metrics_dict['correlation']:.4f} (p={metrics_dict['p_value']:.2e})\n")
            f.write(f"Bias (Mean Error):     {metrics_dict['bias']:.4f} TECU\n")
            f.write(f"Residual Std Dev:      {metrics_dict['std_residual']:.4f} TECU\n\n")
            ...
            f.write(f"Mean Prediction:       {metrics_dict['mean_prediction']:.4f} TECU\n")
            f.write(f"Prediction Std Dev:    {metrics_dict['std_prediction']:.4f} TECU\n\n")
            
            # Error distribution
            f.write("📈 ERROR DISTRIBUTION\n")
            f.write("-" * 40 + "\n")
            f.write(f"Median Abs Error:      {metrics_dict['median_abs_error']:.4f} TECU\n")
            f.write(f"75th Percentile AE:    {metrics_dict['q75_abs_error']:.4f} TECU\n")
            f.write(f"95th Percentile AE:    {metrics_dict['q95_abs_error']:.4f} TECU\n")
            f.write(f"Maximum Abs Error:     {metrics_dict['max_abs_error']:.4f} TECU\n\n")
            
            # Uncertainty metrics (if available)
            if 'mean_uncertainty' in metrics_dict:
                f.write("🔍 UNCERTAINTY ANALYSIS\n")
                f.write("-" * 40 + "\n")
                f.write(f"Mean Uncertainty:      {metrics_dict['mean_uncertainty']:.4f} TECU\n")
                f.write(f"Uncertainty Std Dev:   {metrics_dict['std_uncertainty']:.4f} TECU\n\n")
                
                f.write("📊 COVERAGE ANALYSIS\n")
                f.write("-" * 40 + "\n")
                f.write(f"1σ Coverage:           {metrics_dict['coverage_1sigma']:.2f}% (Expected: 68.27%)\n")
                f.write(f"2σ Coverage:           {metrics_dict['coverage_2sigma']:.2f}% (Expected: 95.45%)\n")
                f.write(f"3σ Coverage:           {metrics_dict['coverage_3sigma']:.2f}% (Expected: 99.73%)\n\n")
                
                if 'uncertainty_calibration' in metrics_dict:
                    f.write("🎯 UNCERTAINTY CALIBRATION\n")
                    f.write("-" * 40 + "\n")
                    f.write(f"Calibration Correlation: {metrics_dict['uncertainty_calibration']:.4f}\n")
                    f.write(f"(How well uncertainty correlates with actual error)\n")
                    f.write(f"Calibration p-value:     {metrics_dict['uncertainty_calibration_p']:.2e}\n\n")
