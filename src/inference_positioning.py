...
class PositioningDataset(Dataset):
    ...
    def __init__(self, config, gnss_data_path, year, doy, test_stations, feature_registry):
        """
        Args:
        ...
        self.target_feature = feature_registry.get_features_by_type(FeatureType.TARGET)[0]
        self.input_features = [f for f in all_features if f != self.target_feature]
        self.swi_features = feature_registry.get_features_by_type(FeatureType.SWI) if feature_registry else []
        ...
        with h5py.File(ccl_path, 'r') as f:
            ...
            station_mask = np.isin(test_data['station'], 
                                   [s.encode('ascii') for s in test_stations])
            
            self.data = test_data[station_mask]
            logger.info(f"Loaded {len(self.data):,} observations from {len(np.unique(self.data['station']))} stations")
...
def run_inference_for_day(config, model, feature_registry, year, doy, test_stations, logger):
    """
    ...
    dataset = PositioningDataset(config, gnss_data_path, year, doy, test_stations, feature_registry)
...
def process_date(args, config, model, feature_registry, test_stations, logger):
    ...
    # Run inference
    results_df = run_inference_for_day(config, model, feature_registry, 
                                       year, doy, test_stations, logger)
