...
def _setup_seed(seed: int, deterministic: bool = False) -> None:
...
class Finetuner(BaseTrainer):
    ...
    def initialize_model(self, model_seed):
        ...
        if finetune_from_scratch:
            ...
            logger.info(
                "   This is useful for single-day training without pretrain phase"
            )
        else:
            # Load pretrained weights
            pretrain_model_dir = os.path.join(self.config["pretrain_folder"], "model")
            pretrain_filename = (
                f"pretrain_{self.config['model']['model_type']}_seed{model_seed}.pth"
            )
            pretrain_checkpoint_path = os.path.join(
                pretrain_model_dir, pretrain_filename
            )
            if not os.path.exists(pretrain_checkpoint_path):
                raise FileNotFoundError(
                    f"Pretrained checkpoint not found: {pretrain_checkpoint_path}"
                )
    ...
    def finetune(self, logger):
        ...
        parallel_ensemble = self.config.get("finetune", {}).get(
            "parallel_ensemble", False
        )

        if train_ensemble and ensemble_size > 1:
            logger.info(
                f"[PAPER] Mao et al. 2025: Training ensemble with {ensemble_size} members"
            )
    ...
    def _train_ensemble_sequential(
        self, train_loader, val_loader, test_loader, training_key, logger
    ):
        ...
        for member_idx in range(self.config["finetune"]["ensemble_size"]):
            logger.info(f"\n{'=' * 80}")
            logger.info(
                f"Ensemble Member {member_idx + 1}/{self.config['finetune']['ensemble_size']}"
            )
            logger.info(f"{'=' * 80}")
            ...
            _setup_seed(ensemble_seed, deterministic=self.config.get("debug", False))
    ...
    def _train_ensemble_parallel(
        ...
        if not _JOBLIB_AVAILABLE:
            logger.warning(
                "joblib not available; falling back to sequential ensemble training."
            )
            self._train_ensemble_sequential(
                train_loader, val_loader, test_loader, training_key, logger
            )
        ...
        if num_gpus == 0:
            logger.warning(
                "⚠️  Parallel ensemble training requested but no GPUs available. Falling back to sequential."
            )
            self._train_ensemble_sequential(
                train_loader, val_loader, test_loader, training_key, logger
            )
        ...
        logger.info(
            f"🚀 Parallel ensemble training: {ensemble_size} members on {num_gpus} GPU(s)"
        )
        ...
        def train_member(member_idx, config_copy, base_seed_val):
            ...
            trainer = BaseTrainer(
                config_copy, logger=None
            )  # logger=None to avoid conflicts
            ...
            print(
                f"✅ Ensemble member {member_idx + 1}/{ensemble_size} completed on GPU {gpu_id}"
            )
        ...
        logger.info(
            f"Starting parallel training of {ensemble_size} ensemble members..."
        )
        Parallel(n_jobs=n_jobs, backend="loky", verbose=10)(
            joblib_delayed(train_member)(idx, configs[idx], base_seed)
