...
def run_inference_analysis(config, experiment_dir, model_path, logger):
    ...
    test_results = trainer.test_model(model, test_loader)
    ...
    bayesian_results, test_df = trainer.bayesian_inference_total_uncertainty(
        model, test_loader, num_samples=100 if 'BNN' in config['model']['model_type'] else 1
    )
    ...
    interpolation_df, extrapolation_df, split_info = trainer.split_test_data_by_date(test_df)
    ...
    if len(interpolation_df) > 1000:
        ...
        plot_test_metrics(interpolation_df, output_dir=interpolation_dir, feature_registry=feature_registry)
    ...
    if len(extrapolation_df) > 1000:
        ...
        plot_test_metrics(extrapolation_df, output_dir=extrapolation_dir, feature_registry=feature_registry)
...
def main():
    ...
    try:
        ...
        metrics, test_df = run_inference_analysis(config, experiment_dir, model_path, logger)
