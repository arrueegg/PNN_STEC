...
def calculate_ipp_coordinates(
    station_lat, station_lon, azimuth, elevation, ipp_height=450.0
):
    """
...
def create_inference_data(
    date_str, hour, lat_grid, lon_grid, elevation, azimuth, config
):
    """
    ...
    for i in range(n_points):
        ipp_lat, ipp_lon = calculate_ipp_coordinates(
            lat_flat[i], lon_flat[i], azimuth, elevation
        )
    ...
    for i in range(n_points):
        ...
        for feature_name in input_features:
            ...
            if feature_name == "year":
                ...
                logger.warning(
                    f"Unknown non-SWI feature {feature_name}, using default value 0.0"
                )
                value = 0.0
        ...
        for feature_name in swi_features:
            if swi_data_loaded is not None and feature_name in swi_data_loaded:
                ...
                if swi_data_loaded is None and i == 0:  # Only log once
                    logger.warning(
                        f"Using default value for SWI feature {feature_name} (no SWI data loaded)"
                    )
            feature_vector.append(value)
            ...
            if is_bayesian:
                ...
                for _ in range(num_samples):
                    ...
                    pred_linear = apply_model_output_transforms(
                        pred_mean_raw.flatten(),
                        pred_var_raw.flatten(),
                        use_log_target,
...
def apply_model_output_transforms(
    pred_mean_raw,
    pred_var_raw,
    use_log_target,
    use_target_standardization,
...
def save_hourly_plot(
    ...
    ax.set_title(
        f"PNN STEC for {date_str} {hour:02d}:00 UTC\nElevation: {elevation}°, Azimuth: {azimuth}°",
        fontweight="bold",
        fontsize=16,
    )
...
def main():
    ...
    # Create global grid
    logger.info(
        f"Creating global grid with resolution {args.lat_res}° x {args.lon_res}°"
    )
    ...
    for hour in hours:
        ...
        features, grid_shape = create_inference_data(
            args.date, hour, lat_grid, lon_grid, args.elevation, args.azimuth, config
        )
        ...
        uncertainty_map = (
            uncertainties.squeeze().numpy().reshape(grid_shape)
        ...
        logger.info(
            f"Saved {base_filename}.png (STEC range: {np.nanmin(stec_map):.2f} - {np.nanmax(stec_map):.2f} TECU)"
        )
    ...
    logger.info("Global map generation complete!")
