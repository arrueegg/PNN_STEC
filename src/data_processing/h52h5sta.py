...
def h5_to_h5_per_station(input_h5, output_root):
    ...
    with h5py.File(input_h5, 'r') as h5f:
        try:
            ...
            print(f"⚠️  Skipping {input_h5}: expected path /{year}/{doy}/all_data not found.")
            return
    ...
    for station in tqdm(stations, desc=f"Stations in {Path(input_h5).name}"):
        ...
        station_key = station.decode('utf-8') if isinstance(station, bytes) else str(station)
...
def main():
    ...
    parser.add_argument('--h5_path', default="/scratch2/arrueegg/WP4/PNN_STEC/test_data", type=str,
                        help='Directory containing source HDF5 files.')
    parser.add_argument('--output_path', default="/scratch2/arrueegg/WP4/PNN_STEC/h5_data", type=str,
