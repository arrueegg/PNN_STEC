    ...
    grid = [
        [
            (
                -180 + col * grid_width,
                -90 + row * grid_height,
                -180 + (col + 1) * grid_width,
                -90 + (row + 1) * grid_height,
            )
...
def split_data_by_grid(
    data, station_counts, train_fraction=0.7, val_fraction=0.15, random_seed=72
):
    ...
    for grid_id in np.unique(data["grid"]):
        grid_data = data[data["grid"] == grid_id].sample(
        ...
        val_stations.extend(
            grid_data.iloc[n_train : n_train + n_val].to_dict(orient="records")
        )
        test_stations.extend(
            grid_data.iloc[n_train + n_val :].to_dict(orient="records")
        )

    return (
        pd.DataFrame(train_stations),
        pd.DataFrame(val_stations),
        pd.DataFrame(test_stations),
    )
...
def save_to_files(train_stations, val_stations, test_stations, output_dir):
    ...
    np.savetxt(
        os.path.join(output_dir, "train_station.list"),
        train_stations["name"].values,
        fmt="%s",
    )
    np.savetxt(
        os.path.join(output_dir, "val_station.list"),
        val_stations["name"].values,
        fmt="%s",
    )
    np.savetxt(
        os.path.join(output_dir, "test_station.list"),
        test_stations["name"].values,
        fmt="%s",
    )
...
def plot_station_distribution(train_stations, val_stations, test_stations, output_file):
    ...
    train_lon_j, train_lat_j = (
        train_stations["lon"].values,
        train_stations["lat"].values,
    )
    ...
    train_scatter = ax.scatter(
        ...
        s=scatter_size,
        c="#215ACC",
        label="Training",  # Slightly darker red
        zorder=3,
        alpha=alpha,
    )
    val_scatter = ax.scatter(
        ...
        s=scatter_size,
        c="#5ACC21",
        label="Validation",  # Slightly darker green
        zorder=4,
        alpha=alpha,
    )
    test_scatter = ax.scatter(
        ...
        s=scatter_size,
        c="#CC215A",
        label="Test",  # Slightly darker blue
        zorder=5,
        alpha=alpha,
    )
    ...
    val_label = (
        f"Validation: {len(val_stations)} ({len(val_stations)/total_stations*100:.1f}%)"
    )
    test_label = (
        f"Test: {len(test_stations)} ({len(test_stations)/total_stations*100:.1f}%)"
    )
    ...
    legend = ax.legend(
        title=f"Total Stations: {total_stations}",
        loc="upper center",
    ...
    print(
        f"Map shows {total_stations} IGS stations distributed across train/val/test sets"
    )
...
def temporal_split():
    ...
    temp_split_map = {
        ...
        2011: {
            "train": [m for m in range(1, 13) if m not in (4, 5)],
            "val": [5],
            "test": [4],
        },
        2012: {
            "train": [m for m in range(1, 13) if m not in (7, 8)],
            "val": [8],
            "test": [7],
        },
        2013: {
            "train": [m for m in range(1, 13) if m not in (10, 11)],
            "val": [11],
            "test": [10],
        },
        2014: {
            "train": [m for m in range(1, 13) if m not in (6, 7)],
            "val": [7],
            "test": [6],
        },
        2015: {
            "train": [m for m in range(1, 13) if m not in (9, 10)],
            "val": [10],
            "test": [9],
        },
        2016: {
            "train": [m for m in range(1, 13) if m not in (12, 1)],
            "val": [1],
            "test": [12],
        },
        2017: {
            "train": [m for m in range(1, 13) if m not in (3, 4)],
            "val": [4],
            "test": [3],
        },
        2018: {
            "train": [m for m in range(1, 13) if m not in (11, 12)],
            "val": [12],
            "test": [11],
        },
        2019: {
            "train": [m for m in range(1, 13) if m not in (2, 3)],
            "val": [3],
            "test": [2],
        },
        2020: {
            "train": [m for m in range(1, 13) if m not in (5, 6)],
            "val": [6],
            "test": [5],
        },
        2021: {
            "train": [m for m in range(1, 13) if m not in (8, 9)],
            "val": [9],
            "test": [8],
        },
        2022: {
    ...
    print(
        f"Temporal splits: train={len(train_str)} months, val={len(val_str)}, test={len(test_str)}"
    )
...
if __name__ == "__main__":
    ...
    if use_grid:
        ...
        train_data, val_data, test_data = split_data_by_grid(
            remaining,
            station_counts,
            train_fraction=train_fraction,
            val_fraction=val_frac_eff,
        )
    else:
    ...
    print(
        f"Train stations: {len(train_data)} ({round((len(train_data)/len(stations))*100, 1)}%), \
    ...
    plot_station_distribution(
        train_data, val_data, test_data, os.path.join(cwd, "stations.png")
    )
