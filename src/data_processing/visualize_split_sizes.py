...
total_combinations = sum(
    len(tg) * len(sg) for tg in temporal_groups for sg in spatial_groups
)
...
temporal_labels = [
    f"{label} ({p*100:.0f}%)" for label, p in zip(split_labels, temporal_props)
]
spatial_labels = [
    f"{label} ({p*100:.0f}%)" for label, p in zip(split_labels, spatial_props)
]
...
for i in range(3):  # temporal (rows)
    for j in range(3):  # spatial (cols)
        ...
        rect = Rectangle(
            (x0, 1 - y1), width, height, facecolor=color, edgecolor="black"
        )
        ax.add_patch(rect)
        ax.text(
            x0 + width / 2,
