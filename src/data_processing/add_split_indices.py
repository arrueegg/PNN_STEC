        ...
        with h5py.File(path, "r+") as h5f:
            ...
            for start in tqdm(range(0, nrows, CHUNK), desc="  ✂ chunk"):
                ...
                for j, rec in enumerate(block):
                    ...
                    idx = start + j
                    if sta in train_stations:
                        train_idx.append(idx)
                    elif sta in val_stations:
                        val_idx.append(idx)
            ...
            print(
                f"  ▶ Wrote indices: {len(train_idx)} train, {len(val_idx)} val, {len(test_idx)} test"
            )
...
if __name__ == "__main__":
    p = argparse.ArgumentParser(
        description="Append train/val/test index lists to daily HDF5 files"
    )
    p.add_argument(
        "--config", required=True, help="YAML config with data paths and filters"
    )
