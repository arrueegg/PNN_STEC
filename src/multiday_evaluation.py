...
# The positioning experiment lives in its own top-level directory. Anchor the
# scripts it owns to this file rather than to the working directory, so the
# subprocess calls below survive being launched from anywhere.
_REPO_ROOT = Path(__file__).resolve().parent.parent
_STEC_CORRECTIONS_SCRIPT = (
    _REPO_ROOT / "positioning" / "scripts" / "generate_stec_corrections.py"
)
_POSITIONING_EVAL_SCRIPT = (
    _REPO_ROOT / "positioning" / "positioning_eval" / "run_positioning_evaluation.py"
)
...
def capture_execution(log_file_path):
    ...
    with open(log_file_path, "a") as f:
        ...
        filtered_stream = LogFilter(
            f, ignore_prefixes=["Bayesian Inference :", "Bayesian Inference:"]
        )
...
def check_experiment_exists(config_path: str) -> Tuple[bool, str]:
    ...
    try:
        ...
        has_root_model = (exp_path / "best_model.pt").exists() or (
            exp_path / "model.pt"
        ).exists()
...
def run_positioning_pipeline(experiment_name: str, year: int, doy: int) -> bool:
    ...
    try:
        ...
        subprocess.run(cmd_eval, cwd=Path(__file__).parent.parent, check=True)
...
def run_comparison(
    ...
    pretrained_baseline: str = None,
    prediction_store_root: str = "predictions",
) -> bool:
    ...
    try:
        ...
        with capture_execution(log_file):
            ...
            args = SimpleNamespace(
                ...
                # Stated explicitly rather than left to the defaults inside the
                # comparison, so it is visible here that every multiday run also
                # populates the per-observation prediction store.
                prediction_store_root=prediction_store_root,
                no_prediction_store=False,
            )
...
def extract_elevation_metrics_from_experiment(
    evaluation_dir: Path,
    ...
    for dataset_type in ["own_vtec_gim", "madrigal_vtec_gim"]:
        ...
        if predictions_file.exists():
            ...
            df["elevation_bin"] = pd.cut(
                df["elevation"], bins=elevation_bins, labels=elevation_bins[:-1]
            )
            ...
            for bin_start, group in df.groupby("elevation_bin"):
                if len(group) > 100:  # Only include bins with sufficient data
                    # Direct STEC metrics
                    stec_rmse = np.sqrt(
                        np.mean((group["true_stec"] - group["stec_pred"]) ** 2)
                    )
                    ...
                    vtec_rmse = np.sqrt(
                        np.mean((group["true_stec"] - group["vtec_model_stec"]) ** 2)
                    )
                    vtec_mae = np.mean(
                        np.abs(group["true_stec"] - group["vtec_model_stec"])
                    )
                    ...
                    if "pretrained_stec_pred" in group.columns:
                        pre_rmse = np.sqrt(
                            np.mean(
                                (group["true_stec"] - group["pretrained_stec_pred"])
                                ** 2
                            )
                        )
                        pre_mae = np.mean(
                            np.abs(group["true_stec"] - group["pretrained_stec_pred"])
                        )
                        pre_bias = np.mean(
                            group["pretrained_stec_pred"] - group["true_stec"]
                        )

                    # GIM metrics
                    gim_rmse = np.sqrt(
                        np.mean((group["true_stec"] - group["gim_stec"]) ** 2)
                    )
...
def generate_aggregate_report(
    batch_results: List[Dict], output_dir: Path, skip_plots: bool = False
):
    ...
    for result in batch_results:
        ...
        evaluation_dir = (
            output_dir / f"{result['year']}_DOY_{result['doy']:03d}" / "evaluation"
        )
            ...
            for baseline in ["Pretrained STEC", "VTEC + Mapping", "IGS GIM + Mapping"]:
                if baseline in pivot_df.columns:
                    ...
                    if not improvement.empty:
                        ...
                        logger.info(
                            f"      Q1 (25%): {improvement.quantile(0.25):.2f}%"
                        )
                        logger.info(
                            f"      Q3 (75%): {improvement.quantile(0.75):.2f}%"
                        )
                        ...
                        logger.info(
                            f"      Direct STEC better on: {better_count}/{total_count} days ({better_count / total_count * 100:.1f}%)"
                        )
...
def generate_aggregate_plots(
    df: pd.DataFrame, batch_results: List[Dict], output_dir: Path
):
    ...
    name_map = {"own_vtec_gim": "Testset", "madrigal_vtec_gim": "Madrigal data"}
    ...
    if "date" in df.columns:
        ...
        logger.warning(
            "No 'date' column found in dataframe, skipping datetime conversion"
        )
    ...
    for dataset in unique_datasets:
        ...
        for model in pivot_df.columns:
            ...
            plt.plot(
                pivot_df.index, pivot_df[model], marker="o", label=model, color=color
            )
    ...
    for dataset in unique_datasets:
        ...
        for metric in ["RMSE", "MAE"]:
                ...
                if not vtec_map.empty and metric in vtec_map.columns:
                    ...
                    if vtec_val != 0:
                        ...
                        improvements.append(
                            {
                                "date_val": date_val,
                                "baseline": "VTEC + Mapping",
                                "improvement": imp_vtec,
                            }
                        )
            ...
            if improvements:
                ...
                prefix = metric.lower()
                filename = (
                    output_dir / f"{prefix}_improvement_by_date_{mapped_name}.png"
                )
                plt.savefig(filename, dpi=300, bbox_inches="tight")
                plt.close()
    ...
    if all_elevation_data:
        ...
        elevation_agg = (
            elevation_df.groupby(["elevation_bin", "dataset"]).agg(agg_dict).round(3)
        )

        # Flatten columns
        elevation_agg.columns = [
            "_".join(col).strip() for col in elevation_agg.columns.values
        ]
        ...
        logger.info(
            f"Loaded existing elevation metrics: {output_dir / 'elevation_metrics.csv'}"
        )
    ...
    if elevation_agg is not None:
        for dataset in elevation_agg["dataset"].unique():
            ...
            has_pretrained = "Pretrained STEC RMSE_mean" in dataset_elev.columns
            ...
            ax2.legend(
                loc="upper center", bbox_to_anchor=(0.5, -0.15), ncol=ncol, frameon=True
            )

            plt.tight_layout()

            combined_fname = (
                output_dir / f"combined_elevation_metrics_{mapped_name}.png"
            )
...
def collect_existing_results(output_base: Path) -> List[Dict]:
    ...
    date_dirs = [
        ...
        if d.is_dir() and "_" in d.name and "DOY" in d.name
    ]
        ...
        try:
            ...
            if len(parts) >= 3 and parts[1] == "DOY":
                ...
                if metrics:  # Only add if we found metrics
                    ...
                    elevation_metrics = extract_elevation_metrics_from_experiment(
                        evaluation_dir
                    )
                    result["elevation_metrics"] = elevation_metrics
...
Date formats supported:
  ...
  - List: "2024-183,2024-184,2024-185"
    ...
    if not args.summary_only and (
        not args.dates or not args.stec_config or not args.vtec_config
    ):
        parser.error(
            "--dates, --stec_config, and --vtec_config are required unless --summary_only is used"
        )
        ...
        if summary_csv.exists():
            ...
            for (date, year, doy), group in df.groupby(["date", "year", "doy"]):
                ...
                for dataset in group["dataset"].unique():
                    ...
                    metrics[dataset] = dataset_df.drop(
                        columns=["date", "year", "doy", "dataset"]
                    ).to_dict("records")
            ...
            logger.info(
                "No existing summary found, collecting from individual evaluations..."
            )
    ...
    for year, doy in dates:
            ...
            try:
                ...
                if metrics_file.exists():
                    ...
                    if metrics and args.pretrained_baseline:
                        ...
                        if not has_pretrain:
                            logger.info(
                                f"⚠️  Existing results found in {date_dir}, but 'Pretrained STEC' is missing. Re-running comparison."
                            )
                            skip_day = False

                    if skip_day and metrics:
                        logger.info(
                            f"✓ Results already found in {date_dir}, skipping day."
                        )
                        result["success"] = True
        ...
        if not args.skip_training:
            ...
            if pretrain_folder is None and (year, doy) == dates[0]:
                ...
                pretrain_success, pretrain_folder = ensure_pretrain_exists(
                    args.stec_config, date_dir
                )
                if not pretrain_success:
                    logger.error(
                        "✗ Failed to ensure pretrain experiment exists, aborting"
                    )
                    break
            ...
            stec_config = create_modified_config(
                args.stec_config,
                year,
                doy,
                date_dir,
            ...
            if args.skip_existing and stec_exists:
                logger.info(
                    f"✓ Found existing STEC experiment: {stec_exp_name}, skipping training"
                )
                stec_success = True
            ...
            vtec_config = create_modified_config(
                args.vtec_config, year, doy, date_dir, is_vtec=True
            )
            ...
            if vtec_exists:  # Original behavior: always skip existing VTEC
                logger.info(
                    f"✓ Found existing VTEC experiment: {vtec_exp_name}, skipping training"
                )
                vtec_success = True
            ...
            def find_exp(config_path, is_vtec):
                try:
                    ...
                    pattern = (
                        f"*STEC_{year}_{str(doy).zfill(3)}*"
                        if not is_vtec
                        else f"*VTEC_{year}_{str(doy).zfill(3)}*"
                    )
                    ...
                    if matches:
                        ...
                        logger.info(
                            f"Using fallback experiment match: {best_match.name}"
                        )
            ...
            if stec_found:
                ...
                logger.error(
                    f"✗ STEC experiment not found (expected: {stec_exp}), skipping {date_str}"
                )
                batch_results.append(result)
                continue
            ...
            if vtec_found:
                ...
                logger.error(
                    f"✗ VTEC experiment not found (expected: {vtec_exp}), skipping {date_str}"
                )
                batch_results.append(result)
                continue
        ...
        if comp_success:
            result["success"] = True
            if not args.skip_comparison:
                result["metrics"] = extract_metrics_from_experiment(
                    date_dir / "evaluation"
                )
            ...
            if args.positioning:
                ...
                pos_success = run_positioning_pipeline(
                    result["stec_experiment"], year, doy
                )
                if pos_success:
