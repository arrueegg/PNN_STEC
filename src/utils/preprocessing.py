...
class DataPreprocessor:
    ...
    def move_files_to_scratch(self, file_paths: Dict[str, List[str]]) -> Dict[str, List[str]]:
        """
        ...
        for split, paths in file_paths.items():
            for path in paths:
                year = path.split('/')[-3]
                doy = path.split('/')[-2]
                scratch_path = os.path.join(self.scratch_dir, year, doy, os.path.basename(path))
    ...
    def get_split_file_lists(self, year: Optional[str] = None, doy: Optional[str] = None) -> Dict[str, List[str]]:
        """
    ...
    def _process_file_chunk(self, chunk_files: List[Tuple], date_to_split: Dict[datetime, str]) -> Dict[str, List]:
        """
        ...
        for dayfile, dt, year, doy, file_key in tqdm(chunk_files, desc="Processing files"):
            try:
    ...
    def _save_chunk_results(self, chunk_idx: int, chunk_data: Dict[str, List]):
        ...
        for split_name, data_list in chunk_data.items():
            ...
            chunk_file = os.path.join(self.temp_dir, f"{split_name}_chunk_{chunk_idx:04d}.h5")
    ...
    def build_split_h5(self) -> bool:
        ...
        total_chunks = (len(all_files_to_process) + self.chunk_size - 1) // self.chunk_size
