...
def integrate_wandb_sweep_config(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    sweep_mappings = {
        ...
        'training.target_weighting.weight_function': ('training', 'target_weighting', 'weight_function'),
        'training.target_weighting.high_value_weight': ('training', 'target_weighting', 'high_value_weight'),
    ...
    if hasattr(wandb.config, 'model_model_type'):
        ...
        if 'BNN' in model_type and not hasattr(wandb.config, 'training_loss_function'):
            ...
            applied_params.append("training.loss_function = GaussianNLLLoss (auto for BNN)")
        elif 'MLP' in model_type and not hasattr(wandb.config, 'training_loss_function'):
    ...
    if cluster_mode:
        ...
        updated_config['data']['scratch_dir'] = '/cluster/work/igp_psr/arrueegg/WP4/PNN_STEC/data/'
        updated_config['data']['GNSS_data_path'] = '/cluster/work/igp_psr/arrueegg/WP4/PNN_STEC/data/STEC_DB_CASDCB'
        updated_config['data']['SWI_data_path'] = '/cluster/work/igp_psr/arrueegg/WP4/PNN_STEC/data/SWI/'
