...
def plot_trends_legacy(df, output_dir):
    """Generate simple trend plots."""


def main():
    parser = argparse.ArgumentParser(description="Multi-Day Positioning Evaluation")
    
    # Changed arguments to match user request (use config)
    parser.add_argument("--config", required=True, 
                        help="Path to base training config used for multiday (e.g., config/config.yaml)")
    ...
    for current_date in dates:
        ...
        # 1. Determine Experiment for this day using CONFIG
        day_experiment = find_finetune_experiment_by_config(base_config, year, doy)
            
        if not day_experiment:
            logger.error(f"Skipping {date_str}: Experiment not found for config + {year}-{doy}")
            continue
            
        logger.info(f"Using experiment: {Path(day_experiment).name}")
        ...
        if not args.skip_inference:
            ...
            if not run_command(inf_cmd, f"Inference for {date_str}", logger):
                continue
        ...
        if run_command(eval_cmd, f"Evaluation for {date_str}", logger):
            ...
            res_path = Path(day_experiment) / "positioning" / "results" / f"{year}{doy:03d}" / "daily_summary.csv"
            if res_path.exists():
                daily_summary_paths.append((current_date, res_path))
    ...
    all_metrics = []
    for date_obj, csv_path in daily_summary_paths:
        try:
