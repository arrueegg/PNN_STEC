...
def load_experiment_config(experiment_folder: str) -> Tuple[Dict, Path]:
    ...
    if "data" in config:
        ...
        if config["data"].get("GNSS_data_path", "").startswith(cluster_base):
            config["data"]["GNSS_data_path"] = config["data"]["GNSS_data_path"].replace(
                cluster_base, local_base
            )

        if config["data"].get("SWI_data_path", "").startswith(cluster_base):
            config["data"]["SWI_data_path"] = config["data"]["SWI_data_path"].replace(
                cluster_base, f"{local_base}/SWI"
            )
...
def load_model_from_checkpoint(
    config: Dict, checkpoint_path: Path, logger
...
def run_inference(
    ...
    if not is_bayesian and num_inference_samples > 1:
        logger.info(
            f"   Note: {model_type} is deterministic - using 1 sample instead of {num_inference_samples}"
        )
...
def apply_mapping_function(
    ...
    """
    logger.info(
        f"📐 Applying {mapping_type} mapping function to {column_prefix} VTEC..."
    )
    ...
    if "pred_stec" in vtec_df.columns:
        ...
        raise KeyError(
            f"Could not find prediction column. Available: {vtec_df.columns.tolist()}"
        )
    ...
    logger.info(
        f"✅ Mapping applied. Mean mapping factor: {mapping_factors.mean():.3f}"
    )
    return vtec_df


def add_gim_comparison(
    test_df: pd.DataFrame, gim_path: str, mapping_type: str, logger
) -> pd.DataFrame:
    """
    ...
    for date_str, indices in tqdm(grouped_data.items(), desc="Processing GIM data"):
        ...
        try:
            ...
            gim_stec = gim_mapper.map_vtec_to_stec(sods, ipp_lats, ipp_lons, elevations)
    ...
    logger.info(
        f"✅ GIM data added for {valid_count:,} / {len(test_df):,} observations ({100 * valid_count / len(test_df):.1f}%)"
    )
...
def compute_metrics(
    predictions: np.ndarray, ground_truth: np.ndarray, mask: Optional[np.ndarray] = None
) -> Dict[str, float]:
...
def save_results(metrics: Dict, test_df: pd.DataFrame, output_dir: Path, args, logger):
    ...
    logger.info("   - detailed_predictions.csv")
    if not args.skip_plots:
        logger.info("   - 5 publication-ready plots")


def write_prediction_store(
    test_df: pd.DataFrame, stec_config: Dict, dataset_type: str, args, logger
) -> None:
    """Persist the full per-observation frame to the parquet prediction store.

    Failures here must not lose an evaluation that already ran, so they are
    logged rather than raised — but loudly, because a silent gap costs a
    re-inference pass to fill.
    """
    year = stec_config.get("year")
    doy = stec_config.get("doy")
    if year is None or doy is None:
        logger.warning(
            "⚠️  No year/doy in config - skipping prediction store "
            "(pretrained multi-year evaluations are stored by inference_testset.py)"
        )
        return

    model_variant = (
        "pretrained_stec" if stec_config.get("mode") == "pretrain" else "finetuned_stec"
    )

    store_df = test_df
    if dataset_type == "madrigal":
        # The Madrigal loader carries no satellite identity - it fills 'sat' with
        # a constant placeholder and leaves slipc/gfphase at zero. Storing those
        # would invite a per-arc groupby that silently returns nonsense, so drop
        # them and let their absence be explicit.
        arc_cols = [c for c in ("sat", "slipc", "gfphase") if c in store_df.columns]
        if arc_cols:
            store_df = store_df.drop(columns=arc_cols)
    ...
    try:
        path = prediction_store.write_predictions(
            ...
            doy=int(doy),
            root=getattr(
                args, "prediction_store_root", prediction_store.DEFAULT_STORE_ROOT
            ),
        )
        logger.info(f"   - prediction store: {path}")
    except Exception as exc:
        logger.error(f"❌ Failed to write prediction store for {year}-{doy}: {exc}")
...
def run_comparison(
    ...
    if stec_config.get("mode") == "finetune":
        if "data" in stec_config and stec_config["data"].get("use_agg_h5"):
            logger.info(
                "⚡ Optimizing: Disabling use_agg_h5 for finetuned model to use day-specific data"
            )
    ...
    if stec_config.get("target", "stec").lower() != "stec":
        raise ValueError(
            f"STEC experiment must have target='stec', got {stec_config.get('target')}"
        )
    ...
    if vtec_experiment:
        ...
        if vtec_config.get("target", "stec").lower() != "vtec":
            logger.warning(
                f"VTEC experiment has target={vtec_config.get('target')}, expected 'vtec'"
            )
...
def main(args=None):
    ...
    if args is None:
        ...
        parser.add_argument(
            ...
            type=str,
            default=str(prediction_store.DEFAULT_STORE_ROOT),
            help="Root directory of the per-observation parquet prediction store",
        )
        parser.add_argument(
            "--no_prediction_store",
            action="store_true",
            help="Skip writing the prediction store (also disables metadata capture)",
        )
    ...
    # inputs, so they only reach the results frame via the metadata channel.
    # Without them the prediction store cannot support per-station or per-arc
    # analysis, and recovering them costs a full re-inference pass.
    if not getattr(args, "no_prediction_store", False):
        stec_config["return_metadata"] = True
        stec_config["metadata_fields"] = ["station", "sat", "slipc", "gfphase"]
    ...
    if stec_config.get("mode") == "finetune":
        ...
        if madrigal_path.exists():
            ...
            logger.warning(
                f"⚠️  Madrigal path not found: {madrigal_path}, skipping Madrigal evaluation"
            )
    else:
        logger.info(
            "ℹ️  Pretrained model detected - Madrigal evaluation only supported for finetuned models"
        )
    ...
    for dataset_type, dataset_name in datasets_to_evaluate:
        ...
        if getattr(args, "reuse_results", False):
            ...
            if args.output_dir:
                check_paths.append(
                    Path(args.output_dir) / comparison_name / "detailed_predictions.csv"
                )
                check_paths.append(
            ...
            for check_path in check_paths:
                if check_path.exists():
                    try:
                        ...
                        logger.info(
                            f"♻️  Found existing results to reuse: {check_path} ({len(reusable_df):,} samples)"
                        )
                        # Ensure it has the required columns
                        required = ["true_stec", "stec_pred", "elevation"]
                        # The legacy detailed_predictions.csv has no uncertainty or
                        # station/satellite columns. Reusing it while building the
                        # store would write a frame that silently cannot support
                        # any stratified analysis, so force re-inference instead.
                        if not getattr(args, "no_prediction_store", False):
                            required = required + [
                                "pred_total_unc",
                                "pred_epistemic_unc",
                        ...
                        if not all(col in reusable_df.columns for col in required):
                            missing = [
                                c for c in required if c not in reusable_df.columns
                            ]
                            logger.warning(
                                f"⚠️  Existing results missing required columns {missing}, will re-evaluate."
                            )
                            reusable_df = None
                            continue
                        else:
                            ...
                            reusable_df.rename(
                                columns={"elevation": "satele"}, inplace=True
                            )
                            break
                    except Exception as e:
                        logger.warning(
                            f"⚠️  Could not read existing results at {check_path}: {e}"
                        )
                        reusable_df = None

        # Prepare test data based on dataset type
        _skip_vtec_gim = (
            False  # Will be set to True in 'own' branch if fast-path applies
        )
        if dataset_type == "madrigal":
            ...
            if test_station_file.exists():
                ...
                logger.info(
                    f"📋 Loaded {len(test_stations)} test stations for filtering"
                )
            else:
                logger.warning(
                    f"⚠️  Test station list not found at {test_station_file}, using all stations"
                )
            ...
            if reusable_df is not None and len(reusable_df) == len(madrigal_dataset):
                logger.info(
                    "⏭️  Skipping STEC model inference (Madrigal), reusing results"
                )
                ...
                if stec_model is None:
                    ...
                    stec_model, stec_registry = load_model_from_checkpoint(
                        stec_config, stec_checkpoint, logger
                    )

                # Run inference on Madrigal data
                logger.info(
                    "🧠 Running STEC model inference on Madrigal observations..."
                )
                test_df = run_inference(
                    stec_model,
                    vtec_madrigal_loader,
                    stec_config,
                    "STEC Model (Madrigal)",
                    logger,
                ...
                test_df.rename(
                    columns={"pred_stec": "stec_pred", "target_stec": "true_stec"},
                    inplace=True,
                )
            ...
            logger.info(
                f"✅ Madrigal inference completed: {len(test_df):,} observations"
            )
            ...
            if stec_config.get("mode") == "finetune":
                logger.info(
                    f"   Using single-day test data for finetuned model (year={stec_config.get('year')}, doy={stec_config.get('doy')})"
                )
            else:
                logger.info("   Using general test.h5 for pretrained model")
            ...
            if needs_pretrained_only:
                ...
                logger.info(
                    "⏭️  Reusing existing STEC/VTEC/GIM results, only running Pretrained STEC inference"
                )
                test_df = reusable_df.copy()
                ...
                vtec_col = (
                    "vtec_model_stec" if "vtec_model_stec" in test_df.columns else None
                )
                ...
                if has_all_existing:
                    logger.info(
                        "⏭️  Skipping STEC model inference (Own Test Set), reusing results"
                    )
                    test_df = reusable_df.copy()
                else:
                    # Need to load STEC model for inference
            ...
            if "pretrained_stec_pred" in test_df.columns:
                ...
                logger.info(
                    "⏭️  Pretrained STEC predictions found in reused results, skipping inference"
                )
                ...
                if not hasattr(main, "_pretrain_stec_model_loaded"):
                    pre_config, pre_dir = load_experiment_config(
                        args.pretrained_stec_experiment
                    )
                    ...
                    if "finetune" in pre_config:
                        pre_config["finetune"]["batchsize"] = max(
                            pre_config["finetune"].get("batchsize", 2048), 4096
                        )
                    if "pretrain" in pre_config:
                        pre_config["pretrain"]["batchsize"] = max(
                            pre_config["pretrain"].get("batchsize", 2048), 4096
                        )

                    # Load pretrained model
                    pre_checkpoint = find_best_checkpoint(pre_dir)
                    pre_model, _ = load_model_from_checkpoint(
                        pre_config, pre_checkpoint, logger
                    )
                ...
                if "finetune" in pre_stec_config:
                    pre_stec_config["finetune"] = {
                        **stec_config["finetune"],
                        "batchsize": max(
                            stec_config["finetune"].get("batchsize", 2048), 4096
                        ),
                    }
                if "pretrain" in pre_stec_config:
                    pre_stec_config["pretrain"] = {
                        **stec_config["pretrain"],
                        "batchsize": max(
                            stec_config["pretrain"].get("batchsize", 2048), 4096
                        ),
                    }
                pre_test_loader = get_test_data_loader(pre_stec_config, logger)

                # Run inference
                pre_df = run_inference(
                    pre_model,
                    pre_test_loader,
                    pre_config,
                ...
                if len(pre_df) != len(test_df):
                    logger.warning(
                        f"⚠️ Pretrained predictions ({len(pre_df)}) don't match STEC model ({len(test_df)})."
                    )
                else:
        ...
        if args.vtec_experiment and not _skip_vtec_gim:
            ...
            if not hasattr(main, "_vtec_model_loaded"):
                ...
                if vtec_config.get("mode") == "finetune":
                    if "data" in vtec_config and vtec_config["data"].get("use_agg_h5"):
                        logger.info(
                            "⚡ Optimizing: Disabling use_agg_h5 for VTEC finetuned model"
                        )
                        vtec_config["data"]["use_agg_h5"] = False
                ...
                if vtec_config.get("target", "vtec").lower() != "vtec":
                    raise ValueError(
                        f"VTEC experiment must have target='vtec', got {vtec_config.get('target')}"
                    )
                ...
                vtec_model, vtec_registry = load_model_from_checkpoint(
                    vtec_config, vtec_checkpoint, logger
                )
            ...
            if dataset_type == "madrigal":
                logger.info(
                    "🔄 Creating VTEC data loader from Madrigal observations..."
                )
            ...
            if len(vtec_df) != len(test_df):
                raise ValueError(
                    f"VTEC predictions ({len(vtec_df)}) don't match STEC predictions ({len(test_df)}). "
                    "Ensure both models use the same test data."
                )

            # Merge VTEC results into test_df (assumes same order from same data loader)
            vtec_cols_to_merge = [
                c for c in vtec_df.columns if c.startswith("vtec_model_stec")
            ]
        ...
        if not args.no_gim and not _skip_vtec_gim:
            if "gim_stec" in test_df.columns:
                ...
                test_df = add_gim_comparison(
                    test_df, args.gim_path, args.mapping_function, logger
                )
                gim_col = "gim_stec"
        ...
        metrics = compare_all_models(
            test_df, "stec_pred", vtec_col, gim_col, logger, pretrain_stec_col
        )
        ...
        # Persist the complete per-observation frame once per dataset. The CSV
        # above keeps its historical narrow schema because the multiday
        # aggregation still reads it; the store is what new analyses read.
        if not getattr(args, "no_prediction_store", False):
            write_prediction_store(test_df, stec_config, dataset_type, args, logger)
