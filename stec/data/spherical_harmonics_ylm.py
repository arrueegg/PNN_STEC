...
Ported byte-for-byte from src/utils/locationencoder/pe/spherical_harmonics_ylm_Arno.py so
`stec/` no longer needs the pre-rebuild source tree to compute spherical-harmonic features
(see stec/data/spherical_harmonics.py). Every Yl{degree}_m{m} the generator produced (degree
0-19) is kept rather than trimmed to what the paper's configs use (degree 0-4 for the STEC
model, 0-15 for the VTEC baseline - see SHConvention in feature_layout.py): this is a
generated lookup table, and hand-trimming it would risk a transcription error for no benefit
over copying it whole.
"""
...
def get_SH(m, degree):
    fname = f"Yl{degree}_m{m}".replace("-", "_minus_")
    return globals()[fname]


def SH(m, degree, phi, theta):
    Ylm = get_SH(m, degree)
...
def Yl16_m0(theta, phi):
    return (
        14862.9380228203 * cos(theta) ** 16
        - 57533.9536367237 * cos(theta) ** 14
        + 90268.7893265838 * cos(theta) ** 12
        - 73552.3468586979 * cos(theta) ** 10
        + 33098.5560864141 * cos(theta) ** 8
        - 8058.77887321386 * cos(theta) ** 6
        + 959.378437287364 * cos(theta) ** 4
        - 43.2802302535653 * cos(theta) ** 2
        + 0.318236987158568
    )
...
def Yl17_m0(theta, phi):
    return (
        29713.0160510757 * cos(theta) ** 17
        - 122453.641907463 * cos(theta) ** 15
        + 207381.167746511 * cos(theta) ** 13
        - 185927.943496872 * cos(theta) ** 11
        + 94685.5267808142 * cos(theta) ** 9
        - 27269.4317128745 * cos(theta) ** 7
        + 4149.69613022003 * cos(theta) ** 5
        - 282.292253756465 * cos(theta) ** 3
        + 5.57155763993023 * cos(theta)
    )
...
def Yl18_m0(theta, phi):
    return (
        59403.1009679377 * cos(theta) ** 18
        - 259676.412802699 * cos(theta) ** 16
        + 472138.932368544 * cos(theta) ** 14
        - 461985.406941263 * cos(theta) ** 12
        + 262853.766018305 * cos(theta) ** 10
        - 87617.9220061016 * cos(theta) ** 8
        + 16355.345441139 * cos(theta) ** 6
        - 1523.7899479322 * cos(theta) ** 4
        + 54.4210695690072 * cos(theta) ** 2
        - 0.318251868824604
    )
...
def Yl19_m0(theta, phi):
    return (
        118765.056929642 * cos(theta) ** 19
        - 548887.154999156 * cos(theta) ** 17
        + 1066409.32971265 * cos(theta) ** 15
        - 1131040.19818008 * cos(theta) ** 13
        + 711460.769822953 * cos(theta) ** 11
        - 269864.429932844 * cos(theta) ** 9
        + 59969.8733184099 * cos(theta) ** 7
        - 7196.38479820918 * cos(theta) ** 5
        + 391.10786946789 * cos(theta) ** 3
        - 6.20806142012525 * cos(theta)
    )
