...
Content-hashing is right for a 200 kB metric CSV and wrong for a 103 GB HDF5 or a 242-day
parquet tree, so the rule is size-dependent: small files are hashed, large files and
directories are summarised by the facts that actually change when they are rewritten
