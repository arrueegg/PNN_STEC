...
| Positioning error, Table 5 | `multiday_results/analyses/positioning_distributions/rebuilt/TABLE5_NUMBERS.md` (+ sibling CSVs in the same directory) | `canonical_for="Table 5"`, `stec.analysis.positioning_distributions`. Median/IQR/P95/P99/exceedance per method, no outcome-based outlier filter — the owner's decided methodology, replacing the mean/10 m-exclusion table (`docs/revision/positioning_reporting.md` has the decision and why). `positioning_summary/rebuilt/overall.csv` still exists and is still declared (superseded for Table 5, `SUPERSEDED_FOR_TABLE5_NOTE` in `stec/analysis/positioning_summary.py`) — kept only for the mean-vs-median sensitivity comparison `positioning_reporting.md` argues from, not as a second Table 5 source. |
...
`daily_metrics` → "Tables 3 and 4", `positioning_distributions` → "Table 5",
...
printed-table deliverable, the latter is superseded for Table 5 and kept only for the
