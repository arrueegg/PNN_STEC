...
package with a 37-stage declared pipeline (`len(stec.pipeline.stages.STAGES)`, confirmed
2026-08-26) and 855 passing tests, replacing `src/` as the thing that produces the paper's
...
| Positioning, Figs 12/13/A1/A2 + Table 5 | `multiday_results/analyses/positioning_coverage/rebuilt/multiday_summary.csv` | `iono` weighting, SINEX ground truth, 4 methods, 2024-05-01→12-31, 37,209 rows post-recovery-sweep. **Resolved 2026-08-24, closing the "open discrepancy" this row used to describe.** `positioning_runs/comparison_3way/` (the published run, 35,652 rows) and `positioning_runs/full_coverage/` (an independent aggregate that happened to hold the same 35,652 rows as of 2026-08-20) were never actually interchangeable: `full_coverage` was what the *pre-rebuild* `src/analysis/positioning_coverage.py` wrote directly, and every downstream consumer (`storm_stratification`, `positioning_robustness`, `common_set_positioning`, `positioning_summary`, `oracle_benchmark`) already preferred it via its own `canonical_positioning_summary()`. The 2026-08-21 results-layout restructure moved this stage's default output to `analyses/positioning_coverage/rebuilt/` without repointing those five readers, so `full_coverage/` silently stopped being regenerated — a file with a real owner that stopped being written, still read as if current. Fixed by repointing `stages.py`'s `POSITIONING` constant and all three modules' `canonical_positioning_summary()` at the stage's actual output, and moving the `positioning_coverage` stage ahead of its five consumers in `stec/pipeline/stages.py` so `check_inputs_are_produced_or_external` enforces the order and a future `experiments/` change propagates automatically instead of silently going stale again. Both `positioning_runs/{comparison_3way,full_coverage}/` are marked superseded (`.superseded.json` sidecars, not deleted) by the `positioning_coverage` stage. |
| Positioning recovery-sweep recompute (2026-08-24) | — | The station-recovery sweep (`weekend-recovery.service`) that finished 2026-08-24 15:07:57 moved iono-weighted coverage from **8,003 / 2,311 / 510 of 10,824** (solved-by-all / all-ML-missing i.e. station absent from STEC DB that day / some-ML-missing i.e. per-method PPPx failure) to **8,195 / 1,591 / 1,067 of 10,853**. The solved-by-all population grew by only 192 station-days (+2.4%): most of the sweep's effect converted "all ML missing" into "some ML missing" rather than completing station-days. **Correcting a claim this row used to make: the remaining 1,591 are not stations the ML methods are structurally unable to serve.** `positioning/geometry/build_recovered_day.py` builds a geometry-only input (RINEX + nav, no DCB/target fields — its own `UNAVAILABLE` constant names what it withholds, and the comment says the model reads none of it) and this exact mechanism already recovered 314 of the original 2,311 all-ML-missing station-days fully and 436 partially (re-derived 2026-08-25 by joining `positioning_runs/full_coverage/coverage.csv` against the current `coverage.csv`). The real blocker for most of the other 1,561: `download_rinex.py` wraps the shell downloader in `subprocess.run(timeout=120)` while the shell script's own retry schedule (5 attempts × two filename formats, sleeps 5/10/20/40/80s) runs past it — confirmed 2026-08-25, all 1,491 "no RINEX" skips in `logs/station_recovery_geometry.log` fire at exactly 120.0–121.0s (zero outliers), and 5/5 freshly re-sampled "absent" station-days had real RINEX on CDDIS, downloaded directly in 5.6–7.5s each. Full writeup, remaining defects and what closing this would be worth: `docs/revision/coverage_recovery_status.md`. Recomputed Table 5 (full recovered set, `positioning_summary`, iono weighting): Direct STEC **1.2229 m over N=8,636** vs IGS GIM **1.6184 m over N=10,837** → **24.4% improvement**, down from the abstract's unmatched 30.9% (1.123 m/8,280 vs 1.626 m/10,809) — recovering more station-days made the unmatched comparison *less* favorable to Direct STEC, not more, because the newly-recovered station-days are harder on average. Recomputed on the common set (`common_set_positioning`, both weightings required): Direct STEC **1.1160 m** vs IGS GIM **1.4007 m**, both **N=7,741** → **20.3% improvement**, essentially unchanged from the pre-sweep common-set number (was 1.115 m/1.400 m over N=7,781). **The full-set and common-set numbers do not converge** (24.4% vs 20.3%): the 1,591 still-absent station-days are disproportionately concentrated (57.3% in 10 stations — `docs/revision/coverage_recovery_status.md` §3) and, per the correction above, are a downloader defect away from closing further, not a fixed structural floor — so this gap is expected to narrow on a re-run, not stay put. Quote 20.3% (N=7,741, matched population) as the defensible apples-to-apples number today; 24.4% (N=8,636/10,837) is the unmatched-population number the abstract's methodology would now produce; both are below the published 30.9%, and both are pre-re-run numbers. Storm/quiet split on the recovered set (`storm_stratification`): 25.4% quiet-day improvement, 19.6% storm-day improvement, both below the module's own documented published 31.9%/26.3%. **Separately found while verifying no day lost stations** (per the `save_daily_summary` shrink-guard history below): three of the 242 per-day source files are truncated on disk with recovery-sweep-era mtimes, unrelated to this aggregation fix — DOY 166 and 176 dropped from ~43 stations to 2 simultaneously across the independent STEC/VTEC/Pretrained experiment trees *and* the GIM arm (a PPPx-level failure for those two days, not an aggregation bug), and DOY 323 dropped from ~41 to 4 stations in the STEC tree only (VTEC and Pretrained are intact for that day). These three days are included in the numbers above as genuinely small samples, not dropped or backfilled — no PPPx re-run was in scope to fix them; they are 3 of 242 days and do not materially move the per-station-day-mean tables above, but would bias a day-count-weighted statistic. The manuscript (`STEC_Modelling/PNN_main.tex`) was not edited — see the memory note on the manuscript freeze; this row is the record of what the current data supports until Phase 8. |
...
[stec/pipeline/stages.py](stec/pipeline/stages.py) (**37 stages, confirmed by
`len(stec.pipeline.stages.STAGES)`, 2026-08-26** — up from 30 on 2026-08-24, 34 later that
day, 35 later still on 2026-08-25 (below); `diagnostic_figures` landed later the same day and
`ionex_rms_benchmark_code` on 2026-08-26, bringing it to 37): a command, what it reads, what it writes,
