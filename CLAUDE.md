...
package with a 39-stage declared pipeline (`len(stec.pipeline.stages.STAGES)`, confirmed
2026-08-28) and 1,092 passing tests, replacing `src/` as the thing that produces the paper's
...
| Positioning, Figs 12/13/A1/A2 + Table 5 | `multiday_results/analyses/positioning_coverage/rebuilt/multiday_summary.csv` | `iono` weighting, SINEX ground truth, 4 methods, 2024-05-01→12-31, **43,101 rows as of the second recovery sweep (2026-08-27)**, up from 37,209 post-first-sweep. `positioning_runs/comparison_3way/` (the published run, 35,652 rows) and `positioning_runs/full_coverage/` (an independent aggregate that happened to hold the same 35,652 rows as of 2026-08-20) were never actually interchangeable: `full_coverage` was what the *pre-rebuild* `src/analysis/positioning_coverage.py` wrote directly, and every downstream consumer (`storm_stratification`, `positioning_robustness`, `common_set_positioning`, `positioning_summary`, `oracle_benchmark`) already preferred it via its own `canonical_positioning_summary()`. The 2026-08-21 results-layout restructure moved this stage's default output to `analyses/positioning_coverage/rebuilt/` without repointing those five readers, so `full_coverage/` silently stopped being regenerated — a file with a real owner that stopped being written, still read as if current. Fixed by repointing `stages.py`'s `POSITIONING` constant and all three modules' `canonical_positioning_summary()` at the stage's actual output, and moving the `positioning_coverage` stage ahead of its five consumers in `stec/pipeline/stages.py` so `check_inputs_are_produced_or_external` enforces the order and a future `experiments/` change propagates automatically instead of silently going stale again. Both `positioning_runs/{comparison_3way,full_coverage}/` are marked superseded (`.superseded.json` sidecars, not deleted) by the `positioning_coverage` stage. A second, deeper instance of the same pattern was found and fixed 2026-08-28 (`b8b0e6e`): `weighting_ablation`, `common_set_positioning` and `oracle_benchmark` never read the experiment trees at all — they read a hardcoded frozen February 2026 snapshot (`positioning_runs/20260216_2052/`) through a `WEIGHTING_RUN` constant no stage produced. `positioning_coverage` now writes `multiday_summary_all_weightings.csv` once both weightings have run, and the three consumers point at that instead, with `weighting_ablation` moved after `positioning_coverage` so the registry enforces the order and their fingerprints invalidate correctly when the underlying data changes — see the "in flight" row below for the elev re-run this enabled. |
| Positioning coverage recovery — outcome (2026-08-28) | `multiday_results/analyses/positioning_coverage/rebuilt/coverage.csv`, `multiday_results/analyses/positioning_summary/rebuilt/overall.csv` | **The recovery effort closed 98.4% of the "structurally unreachable" bucket, and closing it lowered the headline rather than raising it.** Iono-weighted coverage (solved-by-all / all-ML-missing / some-ML-missing, of total): published-era **8,003 / 2,311 / 510 of 10,824** → first sweep (2026-08-24) **8,195 / 1,591 / 1,067 of 10,853** → second sweep (2026-08-27, verified directly against `coverage.csv`'s own `cause` column) **10,598 / 26 / 229 of 10,853**. The geometry sweep that closed most of the remaining gap ran 1,536/1,536 downloads with zero failures once the root cause (`docs/revision/coverage_recovery_status.md`) was fixed: `download_rinex.py` wrapped the shell downloader in `subprocess.run(timeout=120)` while its own retry schedule (5 attempts × 2 filename formats, sleeps 5/10/20/40/80s) runs past that ceiling, against files that were on CDDIS the whole time. **The scientific consequence**: `positioning_summary/rebuilt/overall.csv` now reads Direct STEC **1.5438 m mean / 0.8809 m median over N=10,535** vs IGS GIM **1.6197 m / 1.0877 m over N=10,837** → **mean improvement 4.7%, median improvement 19.0%** — down from the abstract's published 30.9%, down again from the first sweep's unmatched 24.4% (N=8,636/10,837) and matched-common-set 20.3% (N=7,741). Per `multiday_results/analyses/positioning_diagnostics/rebuilt/FINDINGS.md` (built 2026-08-28 by direct invocation, `positioning_diagnostics`/`positioning_diagnostics_figures` were declared as pipeline stages the same morning but have not yet run through the runner — `.pipeline/positioning_diagnostics.json` does not exist): **the recovered station-days are exactly where the model underperforms.** Split by whether the station-day had real STEC-database data: original population **+19.4% mean / +23.5% median (N=8,442)** vs geometry-only recovered **−31.9% mean / −38.5% median (N=2,093)** — the model loses to a global GIM where it has no local observations, and closing the coverage gap pulled in almost exclusively this harder population (only 26 station-days now genuinely absent, down from 1,591). The mean is also non-monotonic in the outlier-exclusion threshold (−0.8% no exclusion, +16.4% at 5 m, +4.7% at 10 m/project rule, +1.7% at 20 m, +0.7% at 50 m) while the median holds 18.6–20.9% across every threshold including none at all — `FINDINGS.md` recommends the paper report the median, or the mean alongside the population split, not the mean alone. 17 of 55 stations with a Direct STEC solution lose to GIM on average; BRMG and LICC have zero Direct STEC solutions at all, unexplained. Storm/quiet split (`storm_stratification/rebuilt/improvement_over_gim.csv`, same fresh coverage, verified directly): Direct STEC vs GIM is now **5.7% quiet / −0.3% storm** — Direct STEC essentially ties GIM on storm days — down from the module's own documented published 31.9%/26.3% and the first sweep's 25.4%/19.6%, the same population effect as above. **`common_set_positioning` (currently N=7,947) and `weighting_ablation` are not yet consistent with this**: `elev-positioning-chain.service` is re-solving elevation-weighted positioning across the newly recovered population (716 day/arm groups, 9,725 station-instances, started 2026-08-28 09:44, ~20h estimated) — `python -m stec.pipeline status` already shows `positioning_coverage`, `weighting_ablation`, `common_set_positioning` and `oracle_benchmark` as "inputs or parameters changed" while it runs; `storm_stratification`/`positioning_robustness`/`positioning_summary` are unaffected (iono-only inputs, already current) and their numbers above are final, not moving. Until the chain lands, treat `common_set_positioning`'s numbers as expected to move, not final. **Separately found while verifying no day lost stations**: three of the 242 per-day source files are truncated on disk with recovery-sweep-era mtimes, unrelated to this fix — DOY 166 and 176 dropped from ~43 stations to 2 simultaneously across the independent STEC/VTEC/Pretrained experiment trees *and* the GIM arm (a PPPx-level failure for those two days, not an aggregation bug), and DOY 323 dropped from ~41 to 4 stations in the STEC tree only (VTEC and Pretrained are intact for that day). These three days are included in the numbers above as genuinely small samples, not dropped or backfilled. The manuscript (`STEC_Modelling/PNN_main.tex`) was not edited — see the memory note on the manuscript freeze; this row is the record of what the current data supports until Phase 8. **2026-08-28: the owner decided the mean-with-10m-exclusion reporting this row describes is itself being replaced** — `docs/revision/positioning_reporting.md` has the decision (distributions/medians, no outcome-based filter), the replacement Table 5, and the three findings (population crossover, storm, attribution) that must reach the manuscript; read it before writing the positioning section. |
| Weighting ablation (elev vs iono) | `multiday_results/positioning_runs/20260216_2052/` (frozen — being replaced) | All six arms: `STEC_elev/iono`, `VTEC_elev/iono`, `gim_elev/iono`. **Frozen February 2026 vintage, six months stale relative to the current recovered population.** As of 2026-08-28 this is mid-replacement, not yet complete: `b8b0e6e` repointed the stage at `positioning_coverage`'s own `multiday_summary_all_weightings.csv` (see the row above) and `elev-positioning-chain.service` is re-solving the elev arm now; until it lands and `weighting_ablation` re-runs, this row's tree is still what's on disk. |
...
**Decided 2026-08-28** (`docs/revision/work_queue.md`'s "Decided: fully-Bayesian model scope"):
scoped as a **single confirmatory comparison** answering R1.2, not a third fully-evaluated
model — no hyperparameter sweep, no positioning, no Madrigal pass. Under matched
initialisation and identical hyperparameters it reached RMSE **15.5389** against the paper
model's **11.6716** (a 52% reduction of the confounded run's 19.7355 gap), while its
uncertainty–error correlation was marginally better (0.5752 vs 0.5682). Its wandb record holds
3 runs, 1 credible (111 epochs), so the letter may say the architecture did not improve
accuracy *under matched conditions* but must **not** say it is inherently inferior — that
stronger claim needs the sweep this decision explicitly declines to run. Related and worth
stating in the methods section: `num_layers` was never varied across the paper model's own
711 runs either, so no implication that depth was searched is supportable.
...
[stec/pipeline/stages.py](stec/pipeline/stages.py) (**39 stages, confirmed by
`len(stec.pipeline.stages.STAGES)`, 2026-08-28** — up from 30 on 2026-08-24, 34 later that
day, 35 later still on 2026-08-25, 37 on 2026-08-26 (`diagnostic_figures` and
`ionex_rms_benchmark_code`); `positioning_diagnostics` and `positioning_diagnostics_figures`
were declared 2026-08-28, bringing it to 39 — though as of that date neither has run through
the pipeline runner yet, only by direct invocation, so `.pipeline/positioning_diagnostics.json`
does not exist): a command, what it reads, what it writes,
