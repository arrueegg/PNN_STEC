...
package with a declared pipeline (`len(stec.pipeline.stages.STAGES)` for the current stage
count — see "The stage pipeline" below) and a passing test suite (`pytest tests/ -q` for the
current count), replacing `src/` as the thing that produces the paper's numbers. `src/` still
exists and still does real work (see "`src/`'s status" below), but it is being retired, not
maintained in parallel. If something below and something in
...
**The rule for this table: a path and the stage that writes it, not the number.** Every cell
below used to also carry the current RMSE/mean/median/row-count as of whatever date the cell
was last hand-edited — three separate documents once quoted three different values for the
same Table 5 median (19.0, 18.5, 18.1) on the same day because the artifact had moved and the
prose hadn't. Read the number from the path; treat any number typed below as a dated snapshot
or a comparison against the *published* manuscript value, never as the current one.

| Purpose | Path | Notes |
|---|---|---|
| STEC metrics backing Tables 3 & 4 | `multiday_results/analyses/daily_metrics/rebuilt/summary.csv` | Declared `canonical_for: "Tables 3 and 4"` (`stec.analysis.daily_metrics`, `python -m stec.pipeline run --only daily_metrics`). Three of the four RMSE rows in this file match the manuscript's published 6.92 / 13.45 / 8.96 (Direct STEC / Pretrained / VTEC+Mapping) exactly; the IGS GIM row reads lower than the published 8.56, because that published number was inflated by the DOY-truncation bug (see Gotchas) and the manuscript needs the correction — read the current value from the CSV, not from here. The `pre_rebuild/` sibling (`daily_metrics/pre_rebuild/summary.csv`) reproduces this file exactly and predates `rebuilt/`'s existence; kept for provenance, not a second number to check against. |
| STEC metrics, original per-day sweep | `multiday_results/stec_evaluation/with_pretrained_baseline/summary/summary_statistics.csv` | The original 4-model × 2-dataset × 242-day sweep this repo has always cited. Frozen at the un-repaired GIM value (the DOY-truncation bug above) — kept for provenance, not the file to quote. |
| Positioning, Figs 12/13 + coverage inputs | `multiday_results/analyses/positioning_coverage/rebuilt/multiday_summary.csv`, `coverage.csv` | `iono` weighting, SINEX ground truth, 4 methods, 2024-05-01→12-31. Row count and the coverage breakdown (solved-by-all / some-ML-missing / all-ML-missing, from `coverage.csv`'s own `cause` column) have moved across four rounds of recovery work — read them from `coverage.csv` directly; `docs/revision/coverage_recovery_status.md` and `docs/revision/work_queue.md` §1–2 carry the history and the diagnosed, no-further-work-expected causes of what's left unsolved. `positioning_runs/comparison_3way/` (the published run) and `positioning_runs/full_coverage/` (a pre-rebuild aggregate) are both marked superseded (`.superseded.json` sidecars, not deleted) by this stage — see the Gotchas entry on the silent-staleness bug that made `full_coverage/` stop being regenerated, and the follow-on fix (`b8b0e6e`) that did the same for `weighting_ablation`/`common_set_positioning`/`oracle_benchmark`, which used to read a hardcoded frozen February 2026 snapshot instead of this stage's output. |
| Positioning error, Tables 6 and 7 | `multiday_results/analyses/positioning_distributions/rebuilt/TABLE5_NUMBERS.md` (+ sibling CSVs in the same directory) | `canonical_for="Tables 6 and 7"`, `stec.analysis.positioning_distributions`. (The `TABLE5_*.md` filenames predate the renumbering — the dSTEC table was inserted as Table 5 on 2026-09-15, pushing the positioning tables to 6, 7 and 8. Numbers verified against `PNN_main_revised.aux`, not by counting `\begin{table}`: one of the nine table environments is a commented-out template example that consumes no number.) Median/IQR/P95/P99/exceedance per method, no outcome-based outlier filter — the owner's decided methodology, replacing the mean/10 m-exclusion table (`docs/revision/positioning_reporting.md` has the decision and why). `positioning_summary/rebuilt/overall.csv` still exists and is still declared (superseded for the positioning-summary table, `SUPERSEDED_FOR_TABLE5_NOTE` in `stec/analysis/positioning_summary.py`) — kept only for the mean-vs-median sensitivity comparison `positioning_reporting.md` argues from, not as a second source for that table. |
| Positioning diagnostics: population split, outlier sensitivity, per-station losses | `multiday_results/analyses/positioning_diagnostics/rebuilt/FINDINGS.md` (+ sibling CSVs) | `stec.analysis.positioning_diagnostics`, `canonical_for=None` (diagnostic, not a manuscript table). Generated by the stage itself — not hand-maintained — from the same CSVs it sits beside, so it cannot drift from them the way this table used to. Read it directly for the population-crossover finding (recovered station-days are exactly where the model underperforms relative to GIM), the outlier-threshold non-monotonicity, and the per-station loss table; `docs/revision/positioning_reporting.md` is the narrative decision record that cites it. |
| Storm/quiet positioning split | `multiday_results/analyses/storm_stratification/rebuilt/improvement_over_gim.csv` | Read the current quiet/storm improvement from this file; the published 31.9%/26.3% storm/quiet figures in the manuscript are a comparison point, not a current one — `docs/revision/manuscript_change_list.md` has the claim-by-claim delta. |
| `common_set_positioning` | `multiday_results/analyses/common_set_positioning/rebuilt/` | `stec.analysis.common_set_positioning`, `canonical_for=None` — this stage answers R1.5 (elevation- vs uncertainty-weighting comparison, both weightings intersected), not a printed manuscript table; there is no lettered appendix in the manuscript (Figures 14–15 are continuously-numbered, not "Table A1"). Reads both weightings via `positioning_coverage`'s `multiday_summary_all_weightings.csv`, so it is only as current as that stage. |
| Weighting ablation (elev vs iono) | `multiday_results/analyses/weighting_ablation/rebuilt/` | `stec.analysis.weighting_ablation`, reads `positioning_coverage`'s `multiday_summary_all_weightings.csv` (fixed `b8b0e6e`, see Gotchas) rather than the frozen `positioning_runs/20260216_2052/` snapshot it used to read — that frozen tree is superseded, not the tree to cite any more. |
...
[stec/pipeline/stages.py](stec/pipeline/stages.py) — run `len(stec.pipeline.stages.STAGES)`
for the current count rather than trusting a number here (it has grown steadily: 30 on
2026-08-24, 39 by 2026-08-28, 43 by 2026-09-14, each addition recorded in `docs/revision/
work_queue.md`'s "Settled" section rather than in this changelog going forward): a command,
what it reads, what it writes, which reviewer comment it answers (`canonical_for` — e.g.
`daily_metrics` → "Tables 3 and 4", `positioning_distributions` → "Tables 6 and 7",
`elevation_metrics_finetuned` → "Figure 11 per-elevation error bars"; `common_set_positioning`
and `positioning_summary` are both `canonical_for=None` — the former answers R1.5 with no
printed-table deliverable, the latter is superseded for that table and kept only for the
mean-vs-median sensitivity comparison, see the canonical-results table above), and the
minimum it must produce to be believed. **No command in this file points at `src/` any
more** — most run `-m stec.analysis.<name>`; the two that still shell out to a standalone script
...
- **`oracle_benchmark` was aligned with the positioning tables on 2026-09-16** (owner
  decision, reversing the earlier "not comparable, by design and permanently" rule recorded
  here). It now restricts to the same four-method/both-weighting common set, drops the 10 m
  outcome-based exclusion, and reports medians. **One difference remains and is permanent
  because it is physical, not a choice:** it uses
  **elev** weighting - the reference STEC carries only a placeholder sigma, so `iono` would
  weight every observation by a constant, and the comparison arms match elev to keep the floor
  fair. Now that it is the *only* remaining difference it reads like a residual detail someone
  could tidy away, which is exactly when it must not be. **A larger caveat, verified from
  `paired_station_days.csv` 2026-09-16:** the oracle's station coverage is severely bimodal -
  52 stations, median 113 days each, but 22 of them contribute 120 station-days between them
  (2.2%) while 14 contribute 3,078 (56%). The floor, and every ratio to it, is dominated by a
  well-covered minority rather than a balanced network sample; restricting to the common set
  does not fix this. Take absolute positioning numbers from the positioning-summary table.
