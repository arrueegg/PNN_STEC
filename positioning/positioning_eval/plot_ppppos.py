...
def read_data(path):
    if path.endswith(".pos"):
        ...
        df = pd.read_csv(
            path, usecols=(1, 2, 3, 4, 5, 9, 10, 11, 12), delim_whitespace=True
        )
        df["hour"] = df["sod"] / 3600
...
def plot_pppx(df, is_plot_all=False, is_scaled=False):
    ...
    if is_plot_all:
        axes[0].plot(
            df["hour"], 100 * df["e"], linewidth=1, color="tab:red", label="East"
        )  # m -> cm
        axes[0].plot(
            df["hour"], 100 * df["n"], linewidth=1, color="tab:green", label="North"
        )
        axes[0].plot(
            df["hour"], 100 * df["u"], linewidth=1, color="tab:blue", label="Up"
        )
        axes[0].legend(ncol=3, loc="upper left")
        axes[0].text(
            0.96,
            0.80,
            f"{std[0]:5.2f} {std[1]:5.2f} {std[2]:5.2f} cm",
            fontsize=12,
        ...
        axes[0].plot(
            df["hour"], 100 * df["e"], ".", markersize=1, color="tab:red", label="East"
        )
        axes[1].plot(
            df["hour"],
            100 * df["n"],
        ...
        axes[2].plot(
            df["hour"], 100 * df["u"], ".", markersize=1, color="tab:blue", label="Up"
        )
        for i in range(3):
            axes[i].text(
                0.96,
    ...
    ylabels = (
        ["Position (cm)", "Clock (us)", "ZTD (mm)", "nsat"]
        if is_plot_all
        else ["East (cm)", "North (cm)", "Up (cm)", "nsat"]
    )
    ylims = (
        [[-10, 10], [-1000, 1000], [1900, 2400], [0, 40]]
        if is_plot_all
        else [[-10, 10], [-10, 10], [-10, 10], [0, 40]]
    )
