...
# save_daily_summary used to carry its own copy of the merge-onto-disk fix (keyed on
# (station, method) - see that function's docstring for why a plain to_csv() overwrite
# destroyed 59 daily_summary*.csv files during the station-day recovery sweep). That copy
# and stec/positioning/summary_writer.py's copy were two implementations of one fix, so
# this module now imports the shared one instead of maintaining its own. stec/ is not an
# installed package, and this file is loaded three different ways (as a flat sibling
# import from run_positioning_evaluation.py, as __main__, and via importlib.util from
# tests/positioning/test_legacy_summary_merge.py) - none of which put the repo root on
# sys.path on their own, so it is inserted explicitly here.
_REPO_ROOT = Path(__file__).resolve().parents[2]
if str(_REPO_ROOT) not in sys.path:
    sys.path.insert(0, str(_REPO_ROOT))

# Re-exported for run_positioning_evaluation.py's `from metrics import (..., save_daily_summary)`
# and for tests/positioning/test_legacy_summary_merge.py, which load this file directly -
# noqa because both names are used only by importers of this module, never within it.
from stec.positioning.summary_writer import (  # noqa: E402, F401
    SummaryShrinkError,
    save_daily_summary,
)
                ...
                if in_estimate:
                    ...
                    if len(parts) >= 9:
                        ...
                        if entry_type in ["STAX", "STAY", "STAZ"]:
                            ...
                            if entry_type == "STAX":
                                coords[station][0] = value
                            elif entry_type == "STAY":
...
def aggregate_daily_metrics(
    results_dir, year, doy, method_name, stations=None, snx_file=None
):
    """
    ...
    if snx_file:
        ...
        print(
            "⚠️  No SINEX file provided - will use day-mean as reference (WARNING: NOT true error!)"
        )
    ...
    for pos_file in pos_files:
        ...
        if require_snx and not ref_pos:
            print(
                f"SKIPPING: Station {station.upper()} not found in SINEX file (no ground-truth coordinates)"
            )
            continue
    ...
    # Reorder columns
    cols = ["station", "method", "year", "doy"] + [
        c for c in metrics_df.columns if c not in ["station", "method", "year", "doy"]
    ]
...
# save_daily_summary and SummaryShrinkError are imported from
# stec.positioning.summary_writer above - see the comment by that import for why.
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--results_dir", type=str, help="Directory with multiple .pos files"
    )
    ...
    parser.add_argument(
        "--method", type=str, choices=["model", "gim"], help="Method name"
    )
    ...
    if args.pos_file:
        ...
        metrics_df = aggregate_daily_metrics(
            args.results_dir, args.year, args.doy, args.method
        )
