...
def download_products(year, doy, output_dir, logger=None):
    ...
    # Reuse what is already on disk. The download script fetches orbits and
    # clocks over FTP from CODE and the SINEX from CDDIS, both of which may be
    # unavailable from a given host - outbound FTP is often firewalled and CDDIS
    # requires Earthdata credentials. Re-running an evaluation on a day whose
    ...
    if not missing:
        ...
        return existing
    if len(missing) < len(existing):
        logger.info(f"Products partially present, missing {missing} - downloading")
...
def find_igs_gim(
    year,
    doy,
    gim_base_path="/home/space/project/2022_shumao_IonoSpatialModeling/07_data/GNSS_ionex",
):
    """
