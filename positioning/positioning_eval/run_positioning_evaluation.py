...
from download_products import download_products, find_igs_gim
    ...
    try:
        ...
        if result.returncode == 0:
            ...
            if pos_files:
                ...
                logger.error("PPPx succeeded but no .pos file found")
                return None
        else:
...
def process_single_station(
    ...
    if stec_csv.exists():
        ...
        pos_file = run_pppx_positioning(
            rinex_file, ini_file, model_output_dir, pppx_executable, logger
        )
    ...
    if igs_gim_path and igs_gim_path.exists():
        ...
        pos_file = run_pppx_positioning(
            rinex_file, ini_file, gim_output_dir, pppx_executable, logger
        )
...
def main():
    ...
    parser.add_argument(
        "--experiment", type=str, required=True, help="Experiment folder name"
    )
    parser.add_argument(
        "--date", type=str, required=True, help="Date to process (YYYY-MM-DD)"
    )
    parser.add_argument(
        "--stations", nargs="+", help="Station names to process (e.g., ZIMM BRUS WTZR)"
    )
    parser.add_argument(
        "--all_test_stations",
        action="store_true",
    ...
    parser.add_argument(
        "--pppx_path",
        type=str,
        # Resolved from this file's own location: the default pointed at
        # ./src/positioning_eval/pppx, which stopped existing when the
        # positioning experiment moved to its own top-level directory, so the
        # script only worked when the path was passed explicitly.
        default=str(Path(__file__).resolve().parent / "pppx"),
    ...
    try:
        ...
        if not args.skip_downloads:
            ...
            if snx_file and not snx_file.exists():
                ...
                logger.error(
                    "   This will cause errors to be computed relative to day-mean instead of ground truth!"
                )
                snx_file = None
            elif snx_file:
        ...
        if igs_gim_path:
            ...
            logger.warning("⚠️  IGS GIM not found - will skip GIM evaluation")
        ...
        if args.parallel > 1:
            ...
            with ProcessPoolExecutor(max_workers=args.parallel) as executor:
                ...
                for future in tqdm(
                    as_completed(futures), total=len(stations), desc="Stations"
                ):
        ...
        # Aggregate metrics
        results_base_dir = (
            experiment_dir / "positioning" / "results" / f"{year}{doy:03d}"
        )
