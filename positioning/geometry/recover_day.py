...
CDDIS_NAV = ("https://cddis.nasa.gov/archive/gnss/data/daily/{year}/{doy:03d}/"
...
EXPERIMENT_PATTERNS = {
    ...
    "Pretrained_STEC": ("Pretrain_STEC_BayesianResNetSTEC_h1024_l4_nh4_v128x4_g32x2"
...
def ensure_nav(year: int, doy: int, workdir: Path) -> Path:
    ...
    result = run(["wget", "--netrc", "--auth-no-challenge=on", "-nv", "-t", "3",
                  "--connect-timeout=15", "--read-timeout=90", "-O", archive, url],
                 capture_output=True)
...
def ensure_bsx(year: int, doy: int) -> Path:
    ...
    if exact:
        return exact[0]
    candidates = sorted(PRODUCTS.glob(f"{year}/*/*DCB.BSX"),
                        key=lambda p: abs(int(p.parent.name) - doy))
    if not candidates:
...
def resolve_experiment(kind: str, doy: int) -> Path | None:
    ...
    matches = [
        path
        for path in sorted((REPO / "experiments").glob(EXPERIMENT_PATTERNS[kind].format(doy=doy)))
...
def run_models(args, stations: list[str]) -> None:
    ...
    for kind in EXPERIMENT_PATTERNS:
        ...
        if experiment is None:
            continue
        corrections = run(["python", "positioning/scripts/generate_stec_corrections.py",
                           "--experiment", experiment.name, "--date", date.strftime("%Y-%m-%d"),
                           "--gnss_path", args.output_root])
        if corrections.returncode != 0:
            logger.error(f"DOY {args.doy}: {kind} corrections failed, skipping its positioning")
            continue
        run(["python", "positioning/positioning_eval/run_positioning_evaluation.py",
             "--experiment", str(experiment.relative_to(REPO)),
             "--date", date.strftime("%Y-%m-%d"), "--stations", *stations,
             "--weight_opt", args.weight_opt, "--parallel", args.parallel])

        if not args.keep_diagnostics:
            results = experiment / "positioning" / "results" / f"{args.year}{args.doy:03d}"
