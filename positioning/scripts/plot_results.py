...
def plot_trends(df, output_dir, threshold_cm=None):
    ...
    if "3d_rms" in df.columns and "method" in df.columns:
        ...
        ordered_methods = sorted(
            ...
            key=lambda x: (
                0
                if "pretrained" in str(x).lower()
                else 1
                if "stec" in str(x).lower()
                else 2
                if "vtec" in str(x).lower()
                else 3
            ),
        )
        ...
        for method in ordered_methods:
            ...
            plt.plot(
                ...
                label=label,
                color=color,
                zorder=3 - ordered_methods.index(method),
            )
...
def plot_model_vs_gim_comparison(df, output_dir, threshold_cm=None):
    ...
    if "method" not in df.columns or "3d_rms" not in df.columns:
        logger.warning(
            "Missing 'method' or '3d_rms' columns, skipping comparison plots"
        )
        return
    ...
    try:
        ...
        pivoted = df_filtered.pivot_table(
            index=pivot_cols, columns="pivot_method", values="3d_rms"
        ...
        for m in model_types:
            if m == "stec":
                ...
                challenger_label = "VTEC + Mapping"
            elif m == "pretrained":
                challenger_label = "Pretrained Direct STEC"
            else:
                challenger_label = str(m)
        ...
        for comp in comparisons:
            ...
            if len(diff_data) > 0:
                ...
                plt.axvspan(
                    0, p99, color="green", alpha=0.05, label=f"{model_label} Better"
                )
                plt.axvspan(
                    p01, 0, color="red", alpha=0.05, label=f"{baseline_label} Better"
                )
                plt.axvline(0, color="k", linestyle="--", linewidth=2)

                plt.xlabel(
                    f"Improvement ({baseline_label} Error - {model_label} Error) [m]\nPositive values = {model_label} is better",
                    fontweight="bold",
                )
                plt.ylabel("Count", fontweight="bold")
                ...
                plt.tight_layout()
                plt.savefig(
                    output_dir / f"comparison_improvement_histogram_{safe_name}.png",
                    dpi=300,
                )
                plt.close()
            ...
            ax = fig.add_axes(
                [0.15, 0.1, 0.7, 0.7]
            )  # Left, Bottom, Width, Height (Fixed Square)

            # Explicit shadow parameter and better colors
            wedges, texts, autotexts = ax.pie(
                [better_count, total_count - better_count],
                labels=[
                    f"{model_label}\nBetter ({win_rate:.1f}%)",
                ...
                colors=[
                    color,
                    "#d62728",
                ],  # Use model color for "better", red for "gim better"
                autopct="%1.1f%%",
                startangle=90,
            ...
            plt.savefig(
                output_dir / f"comparison_win_rate_pie_{safe_name}.png", dpi=300
            )
            plt.close()
...
def generate_comparative_table(df, output_dir, threshold_m=None):
    ...
    for w in stats["weighting"].unique():
        ...
        if gim_mask.any():
            ...
            stats.loc[model_mask, "Imp. [%]"] = (
                (gim_val - stats.loc[model_mask, "3D Mean [m]"]) / gim_val * 100
            )
...
def plot_comparative_metrics(stats_df, output_dir):
    ...
    ax = sns.barplot(
        data=stats_df, x="method", y="3D Mean [m]", hue="weighting", palette="viridis"
    )

    plt.ylabel("Mean 3D RMS Error [m]", fontweight="bold")
    plt.xlabel("Method", fontweight="bold")
    plt.title(
        "Accuracy Comparison: Methods vs. Weighting Strategies",
        fontweight="bold",
        pad=15,
    )
    ...
    if not imp_df.empty:
        ...
        ax = sns.barplot(
            data=imp_df, x="method", y="Imp. [%]", hue="weighting", palette="magma"
        )

        plt.ylabel("Improvement over GIM [%]", fontweight="bold")
        plt.xlabel("Method", fontweight="bold")
        plt.title(
            "Relative Improvement Score vs. IGS Baseline", fontweight="bold", pad=15
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Manual Plotting of Positioning Results"
    )
    parser.add_argument(
        "--input",
        default="multiday_results/positioning/multiday_summary.csv",
        help="Path to input summary CSV",
    )
    parser.add_argument(
        "--exclude_dates", help="Comma-separated list of dates/DOYs to exclude"
    )
    parser.add_argument(
        "--exclude_stations", help="Comma-separated list of station IDs to exclude"
    )
    parser.add_argument(
        "--exclude_threshold",
        type=float,
        help="RMS Error threshold (in meters) to exclude",
    )
    parser.add_argument(
        "--output_dir",
        default="multiday_results/positioning/manual_plots",
