...
def plot_trends(df, output_dir):
    ...
    if "3d_rms" in df.columns and "method" in df.columns:
        ...
        ordered_methods = sorted(
            methods,
            key=lambda x: (
                0 if "stec" in str(x).lower() else 1 if "vtec" in str(x).lower() else 2
            ),
        )
        ...
        plt.title(
            "Overall Positioning Accuracy Distribution", fontweight="bold", pad=15
        )
        ...
        plt.title(
            "Error Cumulative Distribution Function (CDF)", fontweight="bold", pad=15
        )
        ...
        if "2d_rms" in df.columns and "up_rms" in df.columns:
            ...
            plt.plot(
                [0, max_limit], [0, max_limit], "k--", alpha=0.3, label="1:1 Ratio"
            )
...
def process_day(current_date, stec_base_config, vtec_base_config, args):
    ...
    if hasattr(args, "pretrained_baseline") and args.pretrained_baseline:
        pt_path = (
            Path("experiments") / args.pretrained_baseline
            if not Path(args.pretrained_baseline).is_absolute()
            else Path(args.pretrained_baseline)
        )
        if pt_path.exists():
            experiments_to_run.append((str(pt_path), "Pretrained_STEC"))
        else:
            logger.warning(
                f"Skipping Pretrained STEC for {date_str}: Directory {pt_path} not found"
            )
    ...
    for exp_path, model_label in experiments_to_run:
        ...
        if args.only_vtec_inference and "STEC" in model_label:
            run_inf = False
            logger.info("Skipping STEC inference as --only_vtec_inference is set")
        if args.only_stec_inference and model_label == "VTEC":
            run_inf = False
            logger.info("Skipping VTEC inference as --only_stec_inference is set")

        if run_inf:
            # Check if inference output already exists
            correction_dir = (
                Path(exp_path) / "positioning" / "stec_corrections" / f"{year}{doy:03d}"
            )
            ...
            if correction_dir.exists() and any(correction_dir.iterdir()):
                logger.info(
                    f"Skipping inference for {model_label} (Output found at {correction_dir})"
                )
            else:
                inf_cmd = [
                    sys.executable,
                ...
                if not run_command(
                    inf_cmd, f"Inference for {model_label} on {date_str}", logger
                ):
                    continue
        ...
        for w_opt in args.weight_opts:
            ...
            model_type = str(
                vtec_base_config.get("model", {}).get("model_type", "")
            ).lower()
            loss_func = str(
                vtec_base_config.get("training", {}).get("loss_function", "")
            ).lower()

            has_uncertainty = (
                "nll" in loss_func
                or "nll" in model_type
                or "bnn" in model_type
                or "ensemble" in model_type
                or "mcdropout" in model_type
            )

            if (
                current_weight_opt == "iono"
                and not has_uncertainty
                and model_label == "VTEC"
            ):
                logger.info(
                    f"VTEC model is deterministic (no uncertainty). Mapping 'iono' weighting to 'elev' for {model_label}."
                )
                current_weight_opt = "elev"
                # If 'elev' weighting was already processed, we can skip this to avoid duplicates
                if "elev" in args.weight_opts and args.weight_opts.index(
                    "elev"
                ) < args.weight_opts.index("iono"):
                    logger.info(
                        f"Skipping redundant 'iono' (mapped to 'elev') for {model_label}"
                    )
                    continue
            ...
            if res_path.exists() and not args.redo:
                logger.info(
                    f"Skipping evaluation for {model_label} ({current_weight_opt}) (Results found at {res_path})"
                )
                day_results.append(
                    (current_date, res_path, f"{model_label}_{current_weight_opt}")
                )
                continue
            ...
            description = (
                f"Evaluation for {model_label} ({current_weight_opt}) on {date_str}"
            )
...
def main():
    ...
    parser.add_argument(
        "--dates", required=True, help="Date range/list (e.g., 2024-122:2024-130)"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=4,
        help="Number of STATIONS per day to process in parallel",
    )

    parser.add_argument(
        "--skip_inference", action="store_true", help="Skip STEC/VTEC generation step"
    )
    parser.add_argument(
        "--only_vtec_inference",
        action="store_true",
        help="Only generate corrections for VTEC models (skip STEC inference)",
    )
    parser.add_argument(
        "--only_stec_inference",
        action="store_true",
        help="Only generate corrections for STEC models (skip VTEC inference)",
    )
    parser.add_argument(
        "--skip_downloads",
        action="store_true",
        help="Skip GNSS product/RINEX downloads",
    )
    parser.add_argument(
        "--no_cleanup",
        action="store_true",
        help="Do not delete downloaded RINEX/Product files after processing (default is to delete)",
    )
    parser.add_argument(
        "--redo",
        action="store_true",
        help="Force redo of positioning evaluation even if results exist",
    )
    parser.add_argument(
        "--weight_opt",
        type=str,
        default="iono",
        help="Weighting option: elev (elevation) or iono (ionospheric uncertainty). Can be comma-separated list.",
    )
    parser.add_argument(
        "--gnss_path", type=str, help="Custom path to GNSS data (STEC_DB_CASDCB folder)"
    )
        ...
        try:
            for d in args.dates.split(","):
                ...
                if "-" in d and len(d.split("-")) == 3:
                    ...
                    parts = d.split("-")
                    dates.append(
                        datetime(int(parts[0]), 1, 1)
                        + timedelta(days=int(parts[1]) - 1)
                    )
        except ValueError:
    ...
    with ProcessPoolExecutor(max_workers=args.parallel) as executor:
        ...
        for future in tqdm(
            as_completed(futures), total=len(dates), desc="Multiday Progress"
        ):
            try:
    ...
    if all_metrics:
        ...
        if hasattr(args, "output_dir") and args.output_dir:
            ...
            pos_folder_name = (
                "positioning_with_pretrain_"
                if hasattr(args, "pretrained_baseline") and args.pretrained_baseline
                else "positioning_"
            )
            base_output_dir = (
                Path("multiday_results") / f"{pos_folder_name}{run_identifier}"
            )
