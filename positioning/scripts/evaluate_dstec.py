...
def run_model_inference(
    ...
    if eval_config.get("debug", {}).get("single_station") is not None:
        ...
        logger.info(
            f"  🐛 DEBUG: Filtered to station '{debug_station}': {len(test_df):,}/{original_len:,} samples"
        )
...
def compute_dstec_for_pass(
    ...
    dstec_model_var = (
        group["pred_total_unc"].values ** 2 + group.loc[idx_max, "pred_total_unc"] ** 2
    )
...
def save_results(
    ...
    if eval_config["output"]["save_summary_stats"]:
        ...
        pearson_r_model, _ = pearsonr(
            masked_df["dstec_truth"], masked_df["dstec_model"]
        )
        r_squared_model = pearson_r_model**2

        # Check if GIM data is available
        has_gim = (
            "dstec_gim" in masked_df.columns and masked_df["dstec_gim"].notna().any()
        )
        if has_gim:
        ...
        with open(summary_path, "w") as f:
            ...
            if has_gim:
                ...
                f.write("=" * 80 + "\n")
                f.write(
                    f"GIM Performance (across {len(valid_gim_stats)} passes with valid GIM data):\n"
                )
                f.write("=" * 80 + "\n")
                f.write(f"  Mean RMSE: {valid_gim_stats['gim_rmse'].mean():.4f} TECU\n")
                f.write(
                    f"  Median RMSE: {valid_gim_stats['gim_rmse'].median():.4f} TECU\n"
                )
                f.write(f"  Mean MAE: {valid_gim_stats['gim_mae'].mean():.4f} TECU\n")
                f.write(
                    f"  Median MAE: {valid_gim_stats['gim_mae'].median():.4f} TECU\n"
                )
                f.write(
                    f"  Mean Relative Error: {valid_gim_stats['gim_re'].mean():.4f}\n"
                )
                f.write(
                    f"  Median Relative Error: {valid_gim_stats['gim_re'].median():.4f}\n"
                )
                ...
                model_better_rmse = (
                    comparison_stats["model_rmse"] < comparison_stats["gim_rmse"]
                ).sum()
                model_better_mae = (
                    comparison_stats["model_mae"] < comparison_stats["gim_mae"]
                ).sum()
                f.write(
                    f"  Passes where Model RMSE < GIM RMSE: {model_better_rmse}/{len(comparison_stats)} ({100 * model_better_rmse / len(comparison_stats):.1f}%)\n"
                )
                f.write(
                    f"  Passes where Model MAE < GIM MAE: {model_better_mae}/{len(comparison_stats)} ({100 * model_better_mae / len(comparison_stats):.1f}%)\n"
                )
                f.write(
                    f"  Mean RMSE improvement: {(valid_gim_stats['gim_rmse'].mean() - valid_gim_stats['model_rmse'].mean()):.4f} TECU\n"
                )
                f.write(
                    f"  Mean MAE improvement: {(valid_gim_stats['gim_mae'].mean() - valid_gim_stats['model_mae'].mean()):.4f} TECU\n"
                )
...
def generate_plots(
    ...
    if use_percentiles:
        ...
        logger.info(
            f"  Scatter plot: Using {p_lower}-{p_upper}% percentile range, excluding {n_outliers:,} outliers"
        )
    else:
        plot_df = masked_df
    ...
    ax.set_title(
        f"Differential STEC: Model vs Truth\nPearson R = {pearson_r:.4f}, R² = {r_squared:.4f}",
        fontsize=13,
    )
    ax.legend(loc="best")
    ...
    if use_percentiles:
        error_limits = np.percentile(
            masked_df["dstec_model_error"], [100 - p_error, p_error]
        )
        error_mask = (masked_df["dstec_model_error"] >= error_limits[0]) & (
        ...
        logger.info(
            f"  Error histogram: Using ±{p_error}% percentile range, excluding {n_error_outliers:,} outliers"
        )
    else:
        error_plot_df = masked_df

    axes[0].hist(
        error_plot_df["dstec_model_error"], bins=100, alpha=0.7, edgecolor="black"
    )
    ...
    logger.info("  ✓ Saved error analysis plots")
    ...
    logger.info("  ✓ Saved pass statistics plots")
    ...
    axes[0].set_title(
        f"Model vs Truth - Hexbin Density\nR² = {r_squared_full:.4f}", fontsize=13
    )
    ...
    h, xedges, yedges = np.histogram2d(
        masked_df["dstec_truth"], masked_df["dstec_model"], bins=100, range=[xlim, ylim]
    )
    ...
    logger.info(
        f"  ✓ Saved dSTEC density plots (R={pearson_r_full:.4f}, R²={r_squared_full:.4f})"
    )
    ...
    if use_percentiles:
        ...
        error_limits = [
            masked_df["dstec_model_error"].min(),
            masked_df["dstec_model_error"].max(),
        ]
    ...
    axes[0].axhline(
        0, color="red", linestyle="--", linewidth=2, alpha=0.7, label="Zero error"
    )
    ...
    logger.info("  ✓ Saved error vs elevation density plots")
    ...
    if has_gim:
        ...
        if len(gim_df) > 0:
            ...
            pearson_r_gim = np.corrcoef(gim_df["dstec_truth"], gim_df["dstec_gim"])[
                0, 1
            ]
            r_squared_gim = pearson_r_gim**2
            ...
            # Hexbin: GIM vs Truth
            if use_percentiles:
                gim_truth_limits = np.percentile(
                    gim_df["dstec_truth"], [p_lower, p_upper]
                )
            ...
            axes[0].set_title(
                f"GIM vs Truth - Hexbin Density (log scale)\nR² = {r_squared_gim:.4f}",
                fontsize=13,
            )
            axes[0].legend(loc="best")
            axes[0].grid(True, alpha=0.3)
            ...
            im = axes[1].imshow(
                ...
                norm=plt.matplotlib.colors.LogNorm(
                    vmin=max(1, h[h > 0].min()), vmax=h.max()
                ),
                interpolation="bilinear",
            )
            axes[1].plot(
            ...
            plt.tight_layout()
            plt.savefig(
                plots_dir / "dstec_gim_density.png", dpi=150, bbox_inches="tight"
            )
            plt.close()

            logger.info(
                f"  ✓ Saved GIM density plots (R={pearson_r_gim:.4f}, R²={r_squared_gim:.4f})"
            )
            ...
            if use_percentiles:
                gim_error_limits = np.percentile(
                    gim_df["dstec_gim_error"], [100 - p_error, p_error]
                )
            else:
                gim_error_limits = [
                    gim_df["dstec_gim_error"].min(),
                    gim_df["dstec_gim_error"].max(),
                ]
            ...
            plt.tight_layout()
            plt.savefig(
                plots_dir / "gim_error_elevation_density.png",
                dpi=150,
                bbox_inches="tight",
            )
            plt.close()

            logger.info("  ✓ Saved GIM error vs elevation density plots")
            ...
            if use_percentiles:
                abs_error_limits = np.percentile(
                    np.concatenate(
                        [model_gim_df["abs_model_error"], model_gim_df["abs_gim_error"]]
                    ),
                    [0, 99.5],
                )
            else:
                abs_error_limits = [
                    0,
                    max(
                        model_gim_df["abs_model_error"].max(),
            ...
            axes[0].set_title(
                "Model vs GIM: Absolute Error Comparison\n(Below line = Model better)",
                fontsize=13,
            )
            axes[0].legend(loc="best")
            axes[0].grid(True, alpha=0.3)
            ...
            model_gim_df["error_improvement"] = (
                model_gim_df["abs_gim_error"] - model_gim_df["abs_model_error"]
            )
            ...
            axes[1].axvline(
                0, color="red", linestyle="--", linewidth=2, label="No improvement"
            )
            axes[1].axvline(
                model_gim_df["error_improvement"].mean(),
                color="orange",
                linestyle="-",
                linewidth=2,
                label=f"Mean = {model_gim_df['error_improvement'].mean():.3f}",
            )
            axes[1].set_xlabel(
                "Error Improvement (|GIM Error| - |Model Error|) (TECU)", fontsize=12
            )
            axes[1].set_ylabel("Frequency", fontsize=12)
            axes[1].set_title(
                "Model Improvement over GIM\n(Positive = Model better)", fontsize=13
            )
            axes[1].legend()
            axes[1].grid(True, alpha=0.3)
            ...
            plt.tight_layout()
            plt.savefig(
                plots_dir / "model_vs_gim_comparison.png", dpi=150, bbox_inches="tight"
            )
            plt.close()

            logger.info("  ✓ Saved Model vs GIM comparison plots")
            ...
            pearson_r_gim_scatter, _ = pearsonr(
                gim_scatter_df["dstec_truth"], gim_scatter_df["dstec_gim"]
            )
            ...
            ax.set_title(
                f"Differential STEC: GIM vs Truth\nPearson R = {pearson_r_gim_scatter:.4f}, R² = {r_squared_gim_scatter:.4f}",
                fontsize=13,
            )
            ax.legend(loc="best")
            ax.grid(True, alpha=0.3)
            ...
            plt.tight_layout()
            plt.savefig(
                plots_dir / "dstec_gim_scatter.png", dpi=150, bbox_inches="tight"
            )
            plt.close()

            logger.info(
                f"  ✓ Saved GIM scatter plot (R={pearson_r_gim_scatter:.4f}, R²={r_squared_gim_scatter:.4f})"
            )
            ...
            if use_percentiles:
                ...
                gim_error_mask = (gim_df["dstec_gim_error"] >= gim_error_limits[0]) & (
            ...
            axes[0].axvline(
                0, color="red", linestyle="--", linewidth=2, label="Zero error"
            )
            axes[0].set_xlabel("dSTEC GIM Error (TECU)")
            axes[0].set_ylabel("Frequency")
            axes[0].set_title(
                f"GIM Error Distribution\n(±{p_error}th percentile range)"
            )
            axes[0].legend()
            axes[0].grid(True, alpha=0.3)
            ...
            plt.tight_layout()
            plt.savefig(
                plots_dir / "gim_error_analysis.png", dpi=150, bbox_inches="tight"
            )
            plt.close()

            logger.info("  ✓ Saved GIM error analysis plots")
            ...
            if len(gim_stats_df) > 0:
                ...
                axes[1, 0].hist(
                    gim_re_valid, bins=50, alpha=0.7, color="green", edgecolor="black"
                )
                axes[1, 0].set_xlabel("Relative Error")
                axes[1, 0].set_ylabel("Number of Passes")
                ...
                plt.tight_layout()
                plt.savefig(
                    plots_dir / "gim_pass_statistics.png", dpi=150, bbox_inches="tight"
                )
                plt.close()

                logger.info("  ✓ Saved GIM pass statistics plots")
            ...
            if use_percentiles:
                gim_pred_limits_het = np.percentile(
                    gim_df["dstec_gim"], [p_lower, p_upper]
                )
                gim_het_mask = (
            ...
            axes[0, 1].set_title(
                "GIM Residuals vs Fitted (Heteroscedasticity Check)", fontsize=13
            )
            axes[0, 1].grid(True, alpha=0.3)
            ...
            im_gim = axes[1, 0].imshow(
                ...
                aspect="auto",
                norm=plt.matplotlib.colors.LogNorm(
                    vmin=max(1, h_gim[h_gim > 0].min()), vmax=h_gim.max()
                ),
                interpolation="bilinear",
            )
            axes[1, 0].axhline(0, color="red", linestyle="--", linewidth=2, alpha=0.7)
            ...
            axes[1, 1].plot(
                gim_bin_centers, gim_bin_means, "g-", linewidth=2, label="Mean |Error|"
            )
            axes[1, 1].plot(
                gim_bin_centers,
                gim_bin_medians,
            ...
            plt.tight_layout()
            plt.savefig(
                plots_dir / "gim_residual_diagnostics.png", dpi=150, bbox_inches="tight"
            )
            plt.close()

            logger.info("  ✓ Saved GIM residual diagnostic plots")
...
def main():
    ...
    try:
        ...
        logger.info("  Pass size statistics:")
        ...
        if EVALUATION_CONFIG.get("gim", {}).get("enabled", False):
            try:
                ...
                map_type = EVALUATION_CONFIG.get("mapping_function", {}).get(
                    "type", "MSLM"
                )
                ...
                if (
                    len(test_df) > 0
                    and "year" in test_df.columns
                    and "doy" in test_df.columns
                ):
                    first_row = test_df.iloc[0]
                    first_date = datetime(int(first_row["year"]), 1, 1) + timedelta(
                        days=int(first_row["doy"]) - 1
                    )
                    try:
                        gim_mapper.load_gim_data(
        ...
        for pass_id, group in tqdm(
            ...
            result = compute_dstec_for_pass(
                group, EVALUATION_CONFIG, logger, gim_mapper=gim_mapper
            )
