...
def create_sweep(sweep_config_path, project_name="PNN_STEC"):
    ...
    print(
        f"🌐 View sweep at: https://wandb.ai/{entity}/{project_name}/sweeps/{sweep_id}"
    )
...
def submit_agents(sweep_id, num_agents, project_name="PNN_STEC"):
    ...
    print(
        f"   https://wandb.ai/{wandb.api.default_entity}/{project_name}/sweeps/{sweep_id}"
    )
...
def main():
    ...
    parser.add_argument(
        "--config",
        default="config/wandb_sweep_config.yaml",
        help="Path to sweep configuration YAML file",
    )
    parser.add_argument("--project", default="PNN_STEC", help="WandB project name")
    parser.add_argument(
        "--agents", type=int, default=8, help="Number of parallel agents to submit"
    )
    parser.add_argument(
        "--sweep-id", type=str, help="Existing sweep ID (skip creation)"
    )
    parser.add_argument(
        "--create-only",
        action="store_true",
