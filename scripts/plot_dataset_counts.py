...
def scan_database(data_root, start_year=2010, end_year=2024):
    ...
    if missing_files:
        ...
        for date in sorted(missing_files):
            print(f"Missing: {date.strftime('%Y-%m-%d')} (DOY {date.timetuple().tm_yday})")
    ...
    if zero_observations:
        ...
        for date in sorted(zero_observations):
            print(f"Zero obs: {date.strftime('%Y-%m-%d')} (DOY {date.timetuple().tm_yday})")
    ...
    total_expected = sum(366 if year % 4 == 0 and (year % 100 != 0 or year % 400 == 0) else 365 
                        for year in range(start_year, end_year + 1))
...
def plot_observations(dates, counts):
    ...
    bars = ax.bar(years, total_counts, width=0.8, align='center', 
                  color='steelblue', alpha=0.8, edgecolor='navy', linewidth=0.8)
    
    ax.set_xlabel("Year", fontweight='bold')
    ax.set_ylabel("Total STEC Observations", fontweight='bold')
    ax.set_title("Annual STEC Database Coverage\n(2010-2024)", fontweight='bold', pad=20)
    ...
    for i, (year, count) in enumerate(zip(years, total_counts)):
        ax.text(year, count + max_val*0.01, format_count_labels([count])[0], 
    ...
    ax.plot(monthly_dates, total_monthly_counts, linewidth=2, color='darkgreen', 
            marker='o', markersize=4, alpha=0.8)
    
    ax.set_xlabel("Time Period", fontweight='bold')
    ax.set_ylabel("Monthly STEC Observations", fontweight='bold')
    ax.set_title("STEC Database Monthly Coverage Time Series\n(2010-2024)", 
