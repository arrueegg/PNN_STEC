...
def show_transformed_features(feature_registry):
    ...
    for key, value in output_indices.items():
        if isinstance(value, slice):
            ...
            elif any(swi in key for swi in ["Kp", "Sunspot", "Dst", "AE", "ap", "f107"]):
    ...
    # Total dimension
    total_dim = max([idx for _, idx in temporal_transformed + station_transformed + 
                     direction_transformed + ipp_transformed + swi_transformed]) + 1
...
def show_vtec_features(splitter, feature_registry):
    ...
    print(f"These represent the electron content independent of observation geometry.\n")
    ...
    for idx in vtec_indices:
        ...
        elif any(s in feature_name for s in ["Kp", "Sunspot", "Dst", "AE", "ap", "f107"]):
...
def show_geometry_features(splitter, feature_registry):
    ...
    print(f"These represent the geometric transformation from vertical to slant path.\n")
    ...
    if direction:
        ...
        for idx, name in direction:
            component = "vertical" if "e_up" in name else ("eastward" if "e_east" in name else "northward")
...
def show_elevation_extraction(feature_registry):
    ...
    if "e_up" in output_indices:
        ...
        print(f"\n  Transformation:")
        print(f"    1. During collation: azimuth & elevation → (e_up, e_east, e_north)")
        print(f"    2. e_up = sin(elevation_rad)")
        print(f"    3. During splitting: elevation_rad = arcsin(e_up)")
        print(f"\n  This recovered elevation_rad is passed to GeomNet to enforce:")
        print(f"    g(elev) = 1 - sin(elev)")
        print(f"    MF = 1 + g(elev) * softplus(mf_raw)")
    else:
