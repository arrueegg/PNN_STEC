...
def worker(fn):
    ...
    local_stats = {split: {col: RunningStats() for col in COLUMNS} for split in SPLITS}
    local_hist = {col: np.zeros(len(BINS[col]) - 1, dtype=np.int64) for col in COLUMNS}
...
def main():
    ...
    # global accumulators
    global_stats = {split: {col: RunningStats() for col in COLUMNS} for split in SPLITS}
    global_hist = {col: np.zeros(len(BINS[col]) - 1, dtype=np.int64) for col in COLUMNS}
