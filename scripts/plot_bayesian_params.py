...
def extract_bayesian_params(model):
    ...
    for name, module in model.named_modules():
        ...
        if hasattr(module, 'weight_mu') and hasattr(module, 'weight_log_sigma'):
            ...
            if hasattr(module, 'bias_mu') and hasattr(module, 'bias_log_sigma') and module.bias_mu is not None:
                bayesian_params[name]['bias_mu'] = module.bias_mu.detach().cpu().numpy()
                bayesian_params[name]['bias_std'] = torch.exp(module.bias_log_sigma).detach().cpu().numpy()
                bayesian_params[name]['bias_log_sigma'] = module.bias_log_sigma.detach().cpu().numpy()
...
def plot_parameter_distributions(bayesian_params, experiment_name, output_dir):
    ...
    for idx, (layer_name, params) in enumerate(bayesian_params.items()):
        ...
        ax.set_xlabel('Value')
        ax.set_ylabel('Count')
        ax.axvline(weight_mu.mean(), color='red', linestyle='--', 
        ...
        ax.set_xlabel('Value')
        ax.set_ylabel('Count')
        ax.axvline(weight_std.mean(), color='red', linestyle='--',
        ...
        ax.set_xlabel('Ratio')
        ax.set_ylabel('Count')
        ax.axvline(uncertainty_ratio.mean(), color='red', linestyle='--',
...
def print_statistics(bayesian_params):
    ...
    for layer_name, params in bayesian_params.items():
        ...
        print(f"  Weight μ: mean={weight_mu.mean():.6f}, std={weight_mu.std():.6f}, "
              f"min={weight_mu.min():.6f}, max={weight_mu.max():.6f}")
        print(f"  Weight σ: mean={weight_std.mean():.6f}, std={weight_std.std():.6f}, "
              f"min={weight_std.min():.6f}, max={weight_std.max():.6f}")
        
        uncertainty_ratio = weight_std / (np.abs(weight_mu) + 1e-8)
        print(f"  Uncertainty ratio (σ/|μ|): mean={uncertainty_ratio.mean():.6f}, "
...
def main():
    ...
    if not bayesian_params:
        ...
        print("This script is designed for BayesianResNetSTEC or similar models with Bayesian layers.")
        sys.exit(1)
