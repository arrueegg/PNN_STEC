...
def analyze_and_plot(data, output_dir):
    ...
    for idx, (key, values) in enumerate(data.items()):
        ...
        ax.axvline(percs[10], color='purple', linestyle=':', linewidth=2, label=f'10% ({percs[10]:.1f})')
        ax.axvline(percs[33], color='blue', linestyle='--', linewidth=2, label=f'33% ({percs[33]:.1f})')
        ax.axvline(percs[50], color='green', linestyle='-', linewidth=2, label=f'50% ({percs[50]:.1f})')
        ax.axvline(percs[67], color='red', linestyle='--', linewidth=2, label=f'67% ({percs[67]:.1f})')
        ax.axvline(percs[90], color='orange', linestyle=':', linewidth=2, label=f'90% ({percs[90]:.1f})')
