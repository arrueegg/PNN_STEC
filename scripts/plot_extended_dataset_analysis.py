...
def analyze_split_with_omni(split_name, omni_h5, random_state=None):
    ...
    with h5py.File(file_path, "r") as f:
        ...
        samples = {
            col: np.zeros(len(sample_indices), dtype=np.float32) for col in all_columns
        }
        temporal_periods = {}  # Track temporal distribution
        sample_ptr = 0

        # Process samples
        for sample_ptr, sample_idx in enumerate(
            tqdm(sample_indices, desc=f"  Sampling {split_name}")
        ):
...
def main():
    ...
    try:
        ...
        for split in SPLITS:
            stats, samples, temporal = analyze_split_with_omni(
                split, omni_h5, random_state=42
            )
            if stats is None:
                continue
        ...
        with open(comparison_file, "w") as f_out:
            ...
            for col in all_columns:
                ...
                f_out.write("-" * 110 + "\n")
                f_out.write(
                    f"{'Split':<10} {'Count':<12} {'Mean':<12} {'Std':<12} {'Min':<12} {'Max':<12}\n"
                )
                f_out.write("-" * 110 + "\n")
        ...
        for col in tqdm(all_columns, desc="  Histograms"):
            ...
            fig.suptitle(
                f"Distribution of {col} (by split)", fontsize=14, fontweight="bold"
            )
        ...
        for col in tqdm(all_columns, desc="  Overlaps"):
            ...
            for split in SPLITS:
                ...
                if col in ["lat_ipp", "lon_ipp"]:
                    samples_data = samples_data[
                        samples_data != 0
                    ]  # Keep negatives for coords
                else:
                    samples_data = samples_data[
                        samples_data > 0
                    ]  # Remove zero/negative for others
                if len(samples_data) == 0:
                    continue
                ...
                ax.hist(
                    ...
                    bins=bins,
                    alpha=0.5,
                    label=f"{split} (n={len(samples_data):,})",
                    color=colors[split],
                    density=True,
                    edgecolor="black",
                    linewidth=0.5,
            ...
            ax.set_title(
                f"Overlapping Distribution: {col}", fontsize=14, fontweight="bold"
            )
