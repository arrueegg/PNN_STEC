...
> Result values are deliberately not reproduced in this README or in the user manual. They are
> regenerated by the pipeline, and a document that hard-codes a number goes stale silently.
> Three of the shipped results differ from the published paper — the causes are documented in
> [docs/USER_MANUAL.md](docs/USER_MANUAL.md) chapter 10.

## Repository map

| Path | Status | Contents |
|---|---|---|
| `stec/` | **current** | The implementation: config, data, models, training, inference, baselines, analysis, viz, positioning, and a 36-stage pipeline |
| `src/` | legacy | Still the only implementation of training, live inference and spatial maps. Being retired |
| `cli.py` | legacy | Entry point for training, inference, comparison, maps and multi-day sweeps |
| `positioning/` | current | PPPx-based positioning evaluation and the geometry recovery tools |
| `config/` | — | Run configurations, one per model variant, plus wandb sweep definitions |
| `tests/` | — | 1024 tests mirroring the `stec/` layout |
| `verification/` | — | Equivalence gates and independent correctness checks |
| `docs/` | — | Manual, architecture, reproduction guide, and the paper-revision record |
...
The `stec` / `src` split is deliberate and is explained once, properly, in
[docs/USER_MANUAL.md](docs/USER_MANUAL.md) chapter 1.
...
| [docs/MULTIDAY_FILE_STRUCTURE.md](docs/MULTIDAY_FILE_STRUCTURE.md) | Directory layout a multi-day sweep produces |
| [docs/revision/](docs/revision/) | Working record of the paper revision: audits, divergence register, status notes |

`docs/revision/` is history, not instruction. Where it disagrees with a pipeline output, the
